...
import type { RateLimitOptions, RateLimitResult } from '@/lib/server/rate-limit-unified';
/* lib/server/guards.ts */
import type { NextApiRequest, NextApiResponse } from "next";
import { validateAdminAccess, isInvalidAdmin } from "@/lib/server/validation";
import { rateLimit } from "@/lib/server/rateLimit"; import type { RateLimitOptions, RateLimitResult } from '@/lib/server/rate-limit-unified';
// Use the unified rateLimit function
...
export async function requireRateLimit(
  ...
  const result = await rateLimit(`${bucket}:${key}`, ((({ limit, windowMs, keyPrefix: bucket } as any) as unknown as RateLimitOptions) as RateLimitOptions));
  
  if (!result.allowed) {
    res.setHeader("Retry-After", Math.ceil((result as unknown as RateLimitResult).retryAfterMs ?? 0 / 1000).toString());
    
    jsonErr(res, 429, "RATE_LIMITED", "Institutional request threshold exceeded.", {
      retryAfter: Math.ceil((result as unknown as RateLimitResult).retryAfterMs ?? 0 / 1000),
      limit: result.limit,

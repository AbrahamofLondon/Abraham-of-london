...
// Admin-only: run chaos (fault injection) tests against a callable engine.
  ...
  try {
    ...
    if (engine.status !== "PRODUCTION_CALLABLE") {
      return NextResponse.json(
        { ok: false, error: `Engine ${engineId} is ${engine.status} — chaos testing requires PRODUCTION_CALLABLE status` },
        { status: 400 },
      );
    }
    ...
    if (!adapter) {
      return NextResponse.json(
        { ok: false, error: `No chaos adapter registered for engine: ${engineId}` },
        { status: 400 },
      );
    }

    const result = await ChaosRangeService.runChaosTests(engineId, adapter);

    return NextResponse.json({ ok: true, result });
  } catch (err) {
    return NextResponse.json(
      { ok: false, error: err instanceof Error ? err.message : "Chaos run failed" },
      { status: 500 },
    );
  }
}

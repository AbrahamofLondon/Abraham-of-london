// app/api/alignment/enterprise/campaigns/[campaignId]/notify/route.ts
...
export async function POST(
  ...
  try {
    ...
    const snapshot = await getOrganisationSnapshot(campaignId);
    ...
    if (current >= target && !campaign.executiveNotified) {
      ...
      await sendExecutiveBriefNotification({
        ...
        dashboardUrl: `${process.env.NEXT_PUBLIC_SITE_URL}/enterprise/alignment/dashboard/${campaignId}`,
        respondentCount: current
      });

      // 4. Mark as notified in DB to prevent duplicate pings
      // await updateCampaignNotificationStatus(campaignId, true);

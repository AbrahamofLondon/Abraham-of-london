...
// ── Local report shape (matches what the route actually reads) ──────────────
// The service returns a richer object at runtime; this interface covers the
// fields accessed in the handler below. Typed as any-escape where the service
// contract is loose so the compiler does not block the unreachable code.
interface RouteReport {
  state?: string;
  narrative?: { headline?: string; summary?: string; mandate?: string };
  forecast?: {
    resonance?: unknown;
    humanCapital?: unknown;
...
function getCampaignSectors(campaign: any): string[] {
  ...
  return SECTOR_MAP[matchedKey] ?? SECTOR_MAP.default;
}
...
function calculateConfidenceScore(forecast: unknown): number {
  if (!forecast || typeof forecast !== 'object') return 0.75;
  const f = forecast as Record<string, number>;
  const mapeConfidence       = Math.max(0, Math.min(0.3, 1 - ((f.mape ?? 0) / 100)));
  const seasonalityConfidence = (f.seasonalityStrength ?? 0) * 0.3;
  const volatilityConfidence  = Math.max(0, Math.min(0.2, 1 - (f.volatility ?? 0.5)));

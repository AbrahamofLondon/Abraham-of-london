/* app/audit/[id]/audit-form.tsx — CLIENT INTERACTION LAYER */
'use client';

import { useState, useEffect } from 'react';
import { useRouter } from 'next/navigation';
import { Loader2, ShieldCheck, Fingerprint, Lock, EyeOff, AlertCircle } from 'lucide-react';
    ...
    <div className="space-y-10 font-serif">
      ...
      <div className="space-y-5">
        <div className="flex justify-between items-end">
          <div>
            <h3 className="text-sm font-medium tracking-tight text-neutral-700 mb-0.5">1. Resonance Alignment</h3>
            <p className="text-[7px] font-mono text-neutral-400 uppercase tracking-wider">Strategic Congruence</p>
          </div>
        ...
        <input 
          ...
          className="w-full h-1 bg-neutral-200 rounded-none appearance-none cursor-pointer accent-neutral-800"
        />
      </div>
      ...
      <div className="space-y-5">
        <div className="flex justify-between items-end">
          <div>
            <h3 className="text-sm font-medium tracking-tight text-neutral-700 mb-0.5">2. Certainty Coefficient</h3>
            <p className="text-[7px] font-mono text-neutral-400 uppercase tracking-wider">Data Fidelity</p>
          </div>
          <div className="flex flex-col items-end">
      ...
      <div className={`p-5 border transition-all ${isConfirmed ? 'border-neutral-300 bg-neutral-50' : 'border-neutral-200 bg-transparent'}`}>
        <label className="flex items-start gap-3 cursor-pointer">
          <input 
            ...
            className="mt-1 w-3 h-3 accent-neutral-800 border-neutral-300"
          />
          <div className="space-y-1">
            <p className="text-[8px] font-mono uppercase tracking-wider text-neutral-700 font-bold">Integrity Disclosure</p>
      ...
      <button
        ...
        className="group relative w-full py-4 bg-neutral-800 text-white text-[9px] font-mono uppercase tracking-widest overflow-hidden transition-all hover:bg-black disabled:opacity-30"
      >
        <span className={`flex items-center justify-center gap-2 ${loading ? 'opacity-0' : 'opacity-100'}`}>
          Finalize_Protocol <ShieldCheck className="w-3 h-3" />
        </span>
        {loading && (
      ...
      <div className="flex items-center gap-1.5 justify-center">
        ...
        <span className="text-[6px] font-mono text-neutral-400 uppercase tracking-widest">End-to-End Encrypted Node</span>
      </div>
    </div>
  );
}
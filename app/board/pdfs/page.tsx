// ============================================================================
// 1) app/board/pdfs/page.tsx  (CLIENT) — "PDF Intelligence" (PRODUCTION UPGRADE)
//    - Strong typing
//    - Safe parsing + resilient fetch
//    - Human fileSize parsing (string -> bytes)
//    - Polling for generation status (optional)
//    - Security-aware UX (no leaking internals)
// ============================================================================
...
export type PDFItem = {
  ...
  fileSize: string; // human string, e.g. "2.2 MB"
...
function parseHumanBytes(input: string): number {
  // Accepts: "2.2 MB", "800 KB", "512 B", "1.5GB" (case-insensitive)
...
function usePDFDashboard() {
  ...
  const loadPDFs = React.useCallback(async (opts?: { silent?: boolean }) => {
    ...
    try {
      ...
      // Stable ordering: priority -> tier -> title
      ...
      // Auto-select first available if none selected or selected disappeared
  ...
  // OPTIONAL: Light polling while generation is active (keeps UI honest)
  React.useEffect(() => {
  ...
  const tiers = React.useMemo(() => {
    ...
    return ["all", ...ts.sort((a, b) => (order[a] ?? 999) - (order[b] ?? 999))] as Array<
      "all" | PDFTier
    >;
  }, [pdfs]);
  ...
  const generatePDF = React.useCallback(
    async (id: string) => {
      ...
      try {
        ...
        setGenerationStatus({
          message: `✅ Generated: ${resp.filename || id}`,
          type: "success",
        });
      ...
      <div className="max-w-7xl mx-auto">
        <ErrorBoundary>
          ...
          <div className="mb-6 space-y-3">
            ...
            {/* Optional advanced filters (doesn't break your existing PDFFilters component) */}

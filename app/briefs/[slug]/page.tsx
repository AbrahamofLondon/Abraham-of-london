/* app/briefs/[slug]/page.tsx — DYNAMIC (SESSION-AWARE) */
...
import {
  getAllBriefsFromRegistry,
  findBriefBySlug as findBriefInRegistry,
} from "@/lib/briefs/static-registry";
...
/**
 * Find a brief by its URL slug using the static registry.
 * The registry is loaded from public/system/briefs-registry.json which is
 * bundled into the JS at build time — no filesystem access to
 * .contentlayer/generated/ at runtime on Netlify serverless.
 */
function findBriefBySlug(slug: string) {
  return findBriefInRegistry(slug);
}
 ...
 *
 * Must remain dynamic (force-dynamic) because:
 * - getServerSession() reads the session cookie at request time
 * - getInteractionCounts / hasUserInteracted query the live DB
 * - BriefAccessGuard needs real-time tier check per user
 *
 * The brief metadata itself is loaded from the static registry
 * (bundled at build time), not from .contentlayer/generated/ at runtime.
 */
...
// Must be dynamic: session-aware (getServerSession), live DB queries

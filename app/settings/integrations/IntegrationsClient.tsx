...
/**
 * app/settings/integrations/IntegrationsClient.tsx
 *
 * Client component for the OAuth integrations settings page.
 * Imported by the server-component page.tsx wrapper.
 *
 * WHY this split exists:
 *   A pure 'use client' page.tsx (without a server-component wrapper) gets added
 *   to prerender-manifest.json by Next.js. @vercel/next then expects a Lambda for
 *   that route, but finds only a static prerendered file → "Unable to find lambda".
 *
 *   The correct pattern is: server-component page.tsx (force-dynamic) wraps this
 *   client component in <Suspense>. The server page produces a ƒ (Dynamic) Lambda
 *   that Vercel can package, while all the interactive UI lives here.
 */
...
export default function IntegrationsClient() {
  ...
  // Read from window.location.search (client-side only — safe inside useEffect).
  React.useEffect(() => {

...
import { normalizeAnyTier, getTierLabelAny } from "@/lib/access/aol-tier-bridge";
...
interface AccessGateProps {
  ...
  userTier?: AccessTier | string | null | undefined;
...
export default function AccessGate({
  ...
  if (alreadyHasAccess) {
    const continueTier: AccessTier = required === "public" ? "public" : user;
      ...
      <div className="flex items-center justify-center py-12 px-4">
        <div className="w-full max-w-md border border-white/10 bg-zinc-950 shadow-[0_0_50px_rgba(0,0,0,0.8)] animate-in fade-in slide-in-from-bottom-4 duration-700">
          ...
          <div className="p-8 text-center">
            <div className="mx-auto mb-6 flex h-16 w-16 items-center justify-center border border-green-500/40 bg-green-500/10 transition-all duration-500">
            ...
            <p className="mt-4 font-sans text-sm font-light leading-relaxed text-zinc-500 italic">
              {required === "public" ? "This content is public." : "You already have the required clearance."}
            </p>

            <div className="mt-8 inline-flex items-center gap-3 border border-green-500/20 bg-green-500/5 px-4 py-2 font-mono text-[9px] uppercase tracking-[0.3em] text-green-500">
              <Lock size={10} /> {requiredLabel}
            </div>

            <button
              type="button"
              onClick={() => onUnlocked?.(continueTier)}
              className="mt-8 group relative w-full overflow-hidden border border-green-500/20 bg-green-500 px-6 py-4 font-mono text-[10px] font-bold uppercase tracking-[0.3em] text-black transition-all hover:bg-white"
            >
          ...
          <div className="border-t border-white/5 bg-white/[0.01] px-4 py-2 flex justify-between items-center">
            ...
            <div className="flex gap-1">
              <div className="h-1 w-3 bg-green-500/20" />
              <div className="h-1 w-3 bg-green-500/40" />
              <div className="h-1 w-3 bg-green-500/60" />
            </div>
          </div>
        </div>
      </div>
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setTimeout(() => {
        clearAccessCache();
        const unlockedTier = normalizeAnyTier(j?.tier);
        onUnlocked?.(unlockedTier);
      }, 350);
    } catch {
    ...
    <div className="flex items-center justify-center py-12 px-4">
      <div className="w-full max-w-md border border-white/10 bg-zinc-950 shadow-[0_0_50px_rgba(0,0,0,0.8)] animate-in fade-in slide-in-from-bottom-4 duration-700">
        <div className="flex items-center justify-between border-b border-white/10 bg-white/[0.02] px-4 py-2">
          ...
          <div className={clsx("h-1.5 w-1.5 rounded-full animate-pulse", success ? "bg-green-500" : "bg-amber-500")} />
        </div>
        ...
        <div className="p-8 text-center">
          <div
            className={clsx(
              ...
              success ? "border-green-500/40 bg-green-500/10" : "border-amber-500/20 bg-amber-500/5"
            )}
          >
            {success ? (
              <CheckCircle2 className="h-6 w-6 text-green-500 animate-in zoom-in" />
          ...
          <h2 className="font-serif text-2xl italic text-zinc-100">{title}</h2>
          <p className="mt-4 font-sans text-sm font-light leading-relaxed text-zinc-500 italic">"{message}"</p>

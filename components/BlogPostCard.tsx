...
export function BlogPostCard(props: BlogPostCardProps) {
  ...
  return (
    <article className="group h-full overflow-hidden rounded-2xl bg-white shadow-md ring-1 ring-black/10 transition-shadow hover:shadow-lg">
      ...
      <div className="flex h-full flex-col p-5">
        <div className="mb-2 flex flex-wrap items-center gap-2 text-sm text-gray-600">
          {category && <span className="rounded-full border border-forest/20 px-2 py-0.5 text-forest/90">{category}</span>}
          {date && <time dateTime={date} className="text-gray-500">{formatDate(date)}</time>}
          {readTime && (<><span aria-hidden="true">·</span><span className="text-gray-500">{readTime}</span></>)}
        </div>

        <h3 className="text-lg font-semibold leading-snug text-gray-900">
          <Link href={`/blog/${slug}`} className="outline-none transition-colors hover:text-forest focus-visible:rounded focus-visible:ring-2 focus-visible:ring-forest/30">
            {title}
          </Link>
        </h3>

        {excerpt && <p className="mt-2 line-clamp-3 text-sm leading-relaxed text-gray-700">{excerpt}</p>}

/* components/BriefAlert.tsx — Brief Alert Component */
...
type Props = {
  ...
  tone?: "info" | "note" | "tip" | "caution" | "warning" | "danger" | "error" | "success"; // MDX alternative
...
export default function BriefAlert({
  title = "Brief Alert",
  level = "info",
  tone, // Accept MDX tone prop
  children,
  className = "",
  icon,
}: Props) {
  // Map MDX tone to internal level if provided
  let resolvedLevel = level;
  if (tone) {
    const toneMap: Record<string, "info" | "warn" | "danger" | "success"> = {
      info: "info",
      note: "info",
      tip: "success",
      caution: "warn",
    ...
    resolvedLevel = toneMap[tone] || level;
  }

  const s = T[resolvedLevel];

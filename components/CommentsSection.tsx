...
export const HyvorComments: React.FC<HyvorCommentsProps> = ({
  ...
  const pageId = React.useMemo(() => slug || "", [slug]);
  ...
  React.useEffect(() => {
    ...
    return clearTimer;
  }, [retryKey, loadingTimeout, clearTimer, onError]);
  ...
  const handleScriptLoad = React.useCallback(() => {
    ...
    try {
      ...
      const message =
        error instanceof Error ? error.message : "Failed to initialize comments";
      setStatus("error");
      setErrorMessage(message);
      onError?.(message);
    }
  }, [clearTimer, onError, onLoad, pageId, title, useDarkMode, websiteId]);
  ...
  if (!websiteId) {
    return (
      <section
        className={`mt-16 rounded-xl border border-amber-200/60 bg-gradient-to-br from-amber-50/80 to-amber-100/30 px-8 py-8 text-left text-sm text-amber-800 shadow-sm dark:border-amber-800/30 dark:from-amber-900/20 dark:to-amber-800/10 dark:text-amber-200 ${className}`}
        ...
        <div className="flex items-start gap-4">
          ...
          <div>
            ...
            <p className="leading-relaxed text-amber-700 dark:text-amber-300/80">
              The comment system for this article is being prepared. Please check back soon for community discussions.
            </p>
          </div>
        </div>
      </section>
  ...
  return (
    <section
      className={`mt-16 overflow-hidden rounded-xl border border-gray-200/80 bg-white shadow-sm dark:border-gray-800 dark:bg-gray-900/50 transition-all duration-200 ${className}`}

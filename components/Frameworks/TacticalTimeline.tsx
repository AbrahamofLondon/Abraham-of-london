...
type Props = {
  playbook?: Framework["applicationPlaybook"];
};

export const TacticalTimeline: React.FC<Props> = ({ playbook }) => {
    ...
    return (
      <div className="rounded-2xl border border-white/10 bg-white/[0.02] p-8">
        <div className="text-[10px] font-mono uppercase tracking-[0.35em] text-white/35">
          No execution playbook indexed for this dossier.
        </div>
      </div>
    );
  }
      ...
      <div className="flex justify-between items-end mb-10 border-b border-white/5 pb-6">
        <div>
          ...
          <p className="text-zinc-500 text-sm font-mono mt-1 uppercase tracking-widest">
            Velocity_Calibration_Track
          </p>
        </div>
        <div className="hidden md:flex items-center gap-6 text-[10px] font-black uppercase tracking-widest text-zinc-500">
          <span className="flex items-center gap-2">
            <div className="h-2 w-2 rounded-full bg-emerald-500" /> Current Phase
          </span>
          <span className="flex items-center gap-2">
            <div className="h-2 w-2 rounded-full bg-zinc-800" /> Backlog
          </span>
        </div>
      </div>
          ...
          return (
            ...
            >
              <div className="flex-1 pb-12 md:pb-8">
                <div
                  className={`bg-zinc-900/40 border border-white/5 p-6 rounded-2xl hover:border-emerald-500/30 transition-all duration-300 group ${
                    isEven ? "md:mr-12" : "md:ml-12"
                  }`}
                >
                  <div className="flex items-center justify-between mb-3">
                    <span className="text-[10px] font-black text-emerald-500 uppercase tracking-[0.2em]">
                      Phase_{String(step.step).padStart(2, "0")}
                    </span>
              ...
              <div className="absolute left-0 md:left-1/2 md:-translate-x-1/2 flex items-center justify-center z-10">
                <div className="h-8 w-8 rounded-full bg-black border-2 border-zinc-800 flex items-center justify-center shadow-2xl shadow-emerald-500/20">

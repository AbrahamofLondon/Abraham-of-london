...
// FIX: Removed 'async' because this component uses React Hooks (useRouter, useState, useEffect).
export default function Layout({
 ...
 // Lock body scroll + focus first item in drawer
 React.useEffect(() => {
 ...
 return (
  <>
   ...
   <a
    href="#main-content"
    className="sr-only focus:not-sr-only focus:fixed focus:left-4 focus:top-4 focus:z-50 focus:rounded-md focus:bg-white focus:px-3 focus:py-2 focus:shadow dark:focus:bg-[color:var(--color-secondary)]"
   >
    Skip to content
   </a>

   {/* Header */}
   <header className="sticky top-0 z-[60] border-b border-[color:var(--color-on-secondary)/0.1] bg-white/85 backdrop-blur supports-[backdrop-filter]:bg-white/70 dark:bg-[color:var(--color-secondary)]/80">
    <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 md:h-20">
     {/* Brand */}
     <Link href="/" className="inline-flex items-baseline gap-2" prefetch={false}>
   ...
   {/* Sticky CTA on very wide screens only */}
   {!hideCTA && (

  ...
  return (
    <figure className="my-6">
      {w && h ? (
        ...
        <span className={cx("relative block h-96 w-full overflow-hidden rounded-lg shadow-card", className)}>
          <Image
            src={safeSrc}
            alt={altText}
            fill
...
export const YouTube: React.FC<YouTubeProps> = ({ id, url, title, className, start }) => {
  ...
  return (
    <div className={cx("relative w-full overflow-hidden rounded-lg shadow-card", className)} style={{ aspectRatio: "16 / 9" }}>
      <iframe
        src={src.toString()}
      ...
      <div className="flex items-center gap-4">
        {image ? (
          <span className="relative h-16 w-16 overflow-hidden rounded-lg">
            <Image
              src={image}
              alt={title || "Download cover image"}
              fill
              className="object-cover"
              loading="lazy"
              sizes="64px"
        ...
        <div className="min-w-0">
          ...
          {description ? (
            <div className="mt-1 line-clamp-2 text-sm text-[color:var(--color-on-secondary)/0.8]">{description}</div>
          ) : null}
...
function Caption(props: React.HTMLAttributes<HTMLDivElement>) {
  return (
    <div className={cx("mt-2 text-sm text-[color:var(--color-on-secondary)/0.7] italic", props.className)} {...props} />
  );
}
...
const components: MDXComponentsType = {
  ...
  // Normalize headings: page owns <h1>
  h1: (props) => <h2 {...props} />,

...
const A: MDXComponentsType['a'] = ({ href = '', children, className, title }) => {
  const base = 'text-forest underline underline-offset-2 hover:text-softGold transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-forest';
    ...
    <a
      ...
      aria-label={
        typeof children === 'string'
          ? `${children}${isHttp ? ' — opens in new tab' : ''}`
          : undefined
      }
    >
      {children}
...
const Img: MDXComponentsType['img'] = ({
  ...
  return (
    <figure className="my-6">
      ...
      {title && (
        <figcaption className="mt-2 text-sm text-deepCharcoal/70">{title}</figcaption>
      )}
    </figure>
  );
};

...
> = {
  twitter: {
    name: "Twitter",
    color: "hover:bg-blue-500/20 focus:ring-blue-500",
    icon: (
  ...
  linkedin: {
    name: "LinkedIn",
    color: "hover:bg-blue-600/20 focus:ring-blue-600",
    icon: (
      <svg className="h-4 w-4" fill="currentColor" viewBox="0 0 24 24">
  ...
  facebook: {
    name: "Facebook",
    color: "hover:bg-blue-700/20 focus:ring-blue-700",
    icon: (
      <svg className="h-4 w-4" fill="currentColor" viewBox="0 0 24 24">
  ...
  whatsapp: {
    name: "WhatsApp",
    color: "hover:bg-green-500/20 focus:ring-green-500",
    icon: (
      <svg className="h-4 w-4" fill="currentColor" viewBox="0 0 24 24">
...
export default function ShareButtons({
  ...
  variant = "standard",
  size = "md",
  _showLabels, // Prefix with underscore to indicate unused but keep for API compatibility
  ...
  // Move useMemo to top level - never call hooks conditionally
  const resolvedUrl = React.useMemo(() => {
    const trimmed = (url || "").trim();
    if (!trimmed) return "";

    // If it already looks absolute, just use it
    if (/^https?:\/\//i.test(trimmed)) return trimmed;

    // If we're in the browser, resolve against current origin
  ...
  // Simple "non-empty" validation only
  if (!resolvedUrl) {
    console.warn("ShareButtons: Empty URL provided", url);
    if (onError) {
      onError(new Error("Empty URL"), "validation");
    }
    return null;
  }

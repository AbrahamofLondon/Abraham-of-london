...
/* ---------- Inline SVGs (no network fetch → no 404s) ---------- */
function getIcon(kind?: SocialItem["kind"]) {
  const size = 20;
  const color = (kind && BRAND_HEX[kind]) || "#1F2937"; // slate-800

  const common: React.SVGProps<SVGSVGElement> = {
    width: size, height: size, role: "img", "aria-hidden": false,
  };

  switch (kind) {
    case "x":
      return (props: any) => (
        <svg {...common} viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="2" {...props}>
          <path d="M4 4l16 16M20 4L4 20" />
        </svg>
      );
    case "instagram":
      return (props: any) => (
        <svg {...common} viewBox="0 0 24 24" fill={color} {...props}>
    ...
    case "mail":
      return (props: any) => (
        <svg {...common} viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="1.8" {...props}>

"use client";
// components/admin/AppAdminShell.tsx
// App Router admin shell — mirrors AdminLayout.tsx but uses next/navigation hooks.
// Used by app/admin/layout.tsx as the visual wrapper for all app-router admin pages.
      ...
      >
        <span
          className={`text-[9px] font-mono uppercase tracking-[0.18em] ${
            hasActive ? "text-amber-500/70" : "text-white/25"
          }`}
        >
          {section.label}
        </span>
          ...
          {section.items.map((item) => {
            ...
            return (
              ...
              >
                ...
                {item.status === "stub" && (
                  <span className="rounded bg-white/5 px-1 py-0.5 text-[7px] font-mono text-white/15">
                    stub
                  </span>
                )}
                {item.status === "rough" && (
                ...
                {item.status === "broken" && (
                  <span className="rounded bg-red-950/40 px-1 py-0.5 text-[7px] font-mono text-red-500/50">
                    broken
                  </span>
                )}
                {item.status === "deprecated" && (
                  <span className="rounded bg-white/5 px-1 py-0.5 text-[7px] font-mono text-white/10">
                    deprecated
                  </span>
                )}
              </Link>
...
export default function AppAdminShell({
  children,
}: {

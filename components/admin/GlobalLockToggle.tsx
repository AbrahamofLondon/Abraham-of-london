...
export default function GlobalLockToggle() {
  ...
  const toggleSystemLock = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setIsLocked(data.locked ?? !isLocked);
  ...
  return (
    <div
      className={`transition-all duration-500 rounded-2xl border p-6 ${
        isLocked
    ...
    >
      <div className="flex items-center justify-between">
          ...
          <div>
            ...
            <p className="text-[10px] text-slate-500 uppercase tracking-widest mt-1">
              {available === null
                ? "Checking lock state..."
                : !available
                ? "Lock state temporarily unavailable."
                : isLocked
                ? "All non-admin traffic is currently being diverted."
                : "System is accepting all authorised requests."}
            </p>
          </div>
        </div>
        ...
        <button
          ...
          disabled={loading || available === null || available === false}
          className={`relative px-6 py-3 rounded-xl text-[10px] font-black uppercase tracking-[0.2em] transition-all overflow-hidden disabled:opacity-40 ${
            confirming
      ...
      {confirming && (
        <div className="mt-4 pt-4 border-t border-white/5 animate-in fade-in slide-in-from-top-2">
          <p className="text-[10px] text-rose-400 font-mono leading-relaxed italic">
            WARNING: Initiating a lockdown will immediately restrict access to verified Directorate
            IPs only. This action is recorded in the institutional audit trail.
          </p>
          <button

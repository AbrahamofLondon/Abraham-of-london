...
// Define a proper type for the log entry
...
export const SecurityDashboard: React.FC<{ logs: SecurityLog[] }> = ({ logs }) => {
  const metrics = React.useMemo(() => {
    ...
    logs.forEach((log) => {
      // ✅ Get or create entry – TypeScript now knows `entry` is defined
      let entry = map[log.resourceId];
      if (!entry) {
        entry = { resourceId: log.resourceId, totalViews: 0, totalPrints: 0, severityScore: 0 };
        map[log.resourceId] = entry;
      }

      if (log.action === 'VIEW') entry.totalViews++;
      if (log.action === 'BOARD_MEMO_PRINT') entry.totalPrints++;
      if (log.severity === 'HIGH' || log.severity === 'CRITICAL') entry.severityScore += 10;
    });

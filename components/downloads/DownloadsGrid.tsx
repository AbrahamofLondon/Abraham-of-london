          ...
          >
            <div className="flex items-center justify-between gap-4">
              <div>
                ...
                <div className="font-serif text-xl font-semibold text-deepCharcoal">{String(it.title)}</div>
                
                {/* ROBUST: Only render subtitle if it exists and is non-empty */}
                {it.sub?.trim() && <div className="mt-1 text-sm text-[color:var(--color-on-secondary)/0.8]">{it.sub}</div>}
                
                {/* Optional: Display Size if available in the DownloadItem data */}
                {it.size && !it.sub && <div className="mt-1 text-sm text-[color:var(--color-on-secondary)/0.8]">{it.size}</div>}
              </div>

...
interface RelatedEvent {
  ...
  image: string;
  slug: string;
  // Optional draft flag if present in data
  draft?: boolean;
}
...
const RelatedEvents: React.FC<RelatedEventsProps> = ({ events, currentEventId }) => {
  // Filter out drafts, current event, and ensure valid objects
  const validEvents = (events || [])
    .filter(e => e && e.id && e.id !== currentEventId && !e.draft);
  
  const filteredEvents = safeArraySlice(validEvents, 0, 3) as RelatedEvent[];

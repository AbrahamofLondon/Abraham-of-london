  ...
  if (state.status === "saved") {
    return (
      <div style={{ border: `1px solid rgba(100,220,140,0.25)`, backgroundColor: "rgba(100,220,140,0.03)", padding: "1rem 1.25rem" }}>
        <p style={{ ...mono, fontSize: "11px", letterSpacing: "0.18em", textTransform: "uppercase", color: "rgba(100,220,140,0.85)" }}>
          Saved as {state.caseRef}
        </p>
        <Link href={`/decision-centre/case/${encodeURIComponent(state.caseRef)}`}
          style={{ display: "inline-flex", alignItems: "center", gap: "0.4rem", ...mono, fontSize: "11px", letterSpacing: "0.16em", textTransform: "uppercase", color: `${GOLD}DD`, textDecoration: "none", border: `1px solid ${GOLD}44`, backgroundColor: `${GOLD}0A`, padding: "0.6rem 1rem", marginTop: "0.5rem" }}>
    ...
    <div style={{ border: `1px solid ${GOLD}22`, backgroundColor: `${GOLD}05`, padding: "1.25rem" }}>
      ...
      <div className="flex flex-wrap gap-2 items-center">
        <button type="button" onClick={saveCase} disabled={state.status === "saving"}
          style={{ display: "inline-flex", alignItems: "center", gap: "0.5rem", ...mono, fontSize: "11px", letterSpacing: "0.18em", textTransform: "uppercase", color: state.status === "saving" ? "rgba(255,255,255,0.20)" : "#0A0A0A", backgroundColor: state.status === "saving" ? "rgba(255,255,255,0.06)" : GOLD, padding: "0.7rem 1.2rem", border: "none", cursor: state.status === "saving" ? "not-allowed" : "pointer" }}>

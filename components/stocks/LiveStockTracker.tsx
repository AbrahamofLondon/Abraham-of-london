...
// Type guard to validate StockData
...
export default function LiveStockTracker({
  ...
  const [subscribedSymbols, setSubscribedSymbols] = React.useState<Set<string>>(new Set(symbols));
    ...
    process.env.NEXT_PUBLIC_WS_URL || "wss://websocket.example.com/stocks",
    {
      ...
      onMessage: (message) => {
        if (message.type === "price_update" && message.data) {
          // Store data in local variable to avoid "possibly undefined" errors
          ...
          if (isStockData(data)) {
            ...
            console.warn('Received invalid stock data:', data);
          }
        }
      },
    }
  ...
  const sortedStocks = React.useMemo(() => {
    return Array.from(stocks.values()).sort((a, b) => a.symbol.localeCompare(b.symbol));
  }, [stocks]);
    ...
    <div className="w-full max-w-6xl mx-auto p-6 bg-white rounded-lg shadow-lg border border-gray-200">
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-8 gap-4">
        <div className="flex-1">
          <h2 className="text-3xl font-bold text-gray-900 mb-2">Live Stock Tracker</h2>
          <p className="text-gray-600">Real-time stock prices and updates</p>
          {lastUpdate && (
      ...
      <div className="mb-8 p-6 bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl border border-blue-200">
        ...
        <div className="flex justify-between items-center mt-3">
          ...
          <p className="text-sm text-gray-500">Examples: AAPL, TSLA, MSFT, GOOGL, AMZN</p>
        </div>
      </div>
      ...
      {sortedStocks.length > 0 ? (
          ...
          {sortedStocks.map((stock) => (
            ...
            >
              ...
              <div className="space-y-4">
                <div className="text-3xl font-bold text-gray-900">{formatCurrency(stock.price)}</div>
                ...
                >
                  <svg
                    className={`w-4 h-4 mr-1 ${stock.change >= 0 ? "rotate-0" : "rotate-180"}`}
                    fill="currentColor"
                    viewBox="0 0 20 20"
                  >
                    <path
        ...
        <div className="text-center py-16">
          ...
          <h3 className="text-2xl font-semibold text-gray-700 mb-4">
            {isConnected ? "No stocks being tracked" : "Connect to start tracking"}
          </h3>
      ...
      {!isConnected && sortedStocks.length === 0 && (
        <div className="text-center py-12">
          ...
          <p className="text-gray-500 text-lg">Connecting to live market data...</p>
        </div>
      )}
      ...
      <div className="mt-12 pt-6 border-t border-gray-200">
        <div className="flex flex-col md:flex-row justify-between items-center text-sm text-gray-500">
          ...
          <div className="mt-2 md:mt-0">
            <span className="text-xs bg-gray-100 px-2 py-1 rounded">Live updates every 1–2 seconds</span>
          </div>
        </div>
      </div>
    </div>

...
type EventPageProps = {
  event: EventMeta & {
    slug: string;
    tags?: string[] | null;
    // tolerate either name in front-matter at runtime (we don't request coverImage from the loader)
...
function EventPage({ event, contentSource }: EventPageProps) {
  ...
  // Use whichever the content provides; we don't request coverImage in FieldKeys
  const relImage = event.coverImage ?? event.heroImage ?? undefined;
...
export async function getStaticProps({ params }: { params: { slug: string } }) {
  // Request ONLY supported FieldKeys from your loader

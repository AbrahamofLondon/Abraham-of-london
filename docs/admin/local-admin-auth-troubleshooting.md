...
At least one authorised admin email must be present in the bootstrap admin allow-list. The current bootstrap list is maintained in `lib/access/admin-emails.ts` and includes official admin addresses plus private fallback admin addresses.

`ADMIN_USER_EMAILS` is not currently read by the local admin guard. Multiple admin identities are controlled by the bootstrap allow-list in `lib/access/admin-emails.ts`; `ADMIN_USER_EMAIL` is only used by the optional credentials provider.
...
3. Confirm the login email appears in `lib/access/admin-emails.ts`.

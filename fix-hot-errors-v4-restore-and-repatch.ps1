...
function Get-LatestBackupDir {
  ...
  $dirs = Get-ChildItem $root -Directory -Filter "ts-hotfix-*" | Sort-Object Name -Descending
  if (-not $dirs -or $dirs.Count -eq 0) { throw "No ts-hotfix backup directories found in _fixbak." }
  return $dirs[0].FullName
}

function Restore-FromBackup([string]$BackupDir, [string]$RepoPath) {
  # backup file names are path-sanitized: replace : \ / with _
  $safe = ($RepoPath -replace "[:\\\/]", "_")
  $bak = Join-Path $BackupDir $safe
  if (-not (Test-Path $bak)) {
    throw "Backup not found for $RepoPath. Expected: $bak"
  }
...
"@
  ...
  # C) If the function signature was not upgraded (maybe different spacing), do a looser upgrade:
...
if ($LASTEXITCODE -eq 0) {
  ...
  Write-Host "`n❌ TypeScript still failing. v4 backups at: $newBak" -ForegroundColor Yellow
  exit 1
}
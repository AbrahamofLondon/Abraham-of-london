...
function Get-LatestBackupDir {
  ...
  # Force array no matter what comes back
  $dirsRaw = Get-ChildItem -Path $root -Directory -Filter "ts-hotfix-*" -ErrorAction SilentlyContinue
  $dirs = @($dirsRaw)

  # Robust count (works even if something weird sneaks in)
  $count = ($dirs | Measure-Object).Count
  if ($count -eq 0) { throw "No ts-hotfix backup directories found in _fixbak." }

  # Sort and pick newest by name (your naming is sortable)
...
function Get-BackupFilePath([string]$BackupDir, [string]$RepoPath) {
  ...
  $rel = $abs
  if ($abs.StartsWith($root)) { $rel = $abs.Substring($root.Length).TrimStart("\","/") }
...
$newBak = ".\_fixbak\ts-hotfix-v4_2-$stamp"
New-Item -ItemType Directory -Force -Path $newBak | Out-Null
foreach ($t in $targets) { Backup-File $t $newBak }
Write-Host "Stage v4.2 backups at: $newBak" -ForegroundColor DarkGray
...
if ($LASTEXITCODE -eq 0) {
  ...
  Write-Host "`n❌ TypeScript still failing. v4.2 backups at: $newBak" -ForegroundColor Yellow
  exit 1
}
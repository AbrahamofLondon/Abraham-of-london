# fix-resources-duplicate.ps1
# Fix the duplicate variable declaration in resources.ts

Write-Host "=== Fixing resources.ts duplicate variable issue ===" -ForegroundColor Cyan
...
Set-Content -Path "lib/resources.ts" -Value $resourcesContent
Write-Host "✓ Fixed duplicate coverImage variable in resources.ts" -ForegroundColor Green

Write-Host "`n=== Running type check to verify fix ===" -ForegroundColor Cyan
pnpm run typecheck:safe 2>&1 | Select-String -Pattern "error TS"

Write-Host "`n=== Attempting build again ===" -ForegroundColor Cyan

...
describe("admin email resolver", () => {
  ...
  it("keeps hardcoded bootstrap addresses", () => {
    expect(isResolvedAdminEmail("INFO@ABRAHAMOFLONDON.ORG", {} as NodeJS.ProcessEnv)).toBe(true);
    expect(isResolvedAdminEmail("reader@example.com", {} as NodeJS.ProcessEnv)).toBe(false);
  });

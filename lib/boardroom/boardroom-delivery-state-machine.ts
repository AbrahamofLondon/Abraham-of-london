 ...
 *
 * Boardroom Brief delivery state machine — strict, auditable, governed.
 *
 * States:
 *   paid                     — payment confirmed, order created
 *   case_stubs_created       — ProductArtifact, FalsificationEntry, OutcomeHypothesis stubs exist
 *   draft_generated          — case study / dossier draft exists in DB
 *   awaiting_operator_review — draft ready for admin review
 *   approved_for_delivery    — admin approved; customer access can be generated
 *   customer_access_ready    — secure access token created; customer link available
 *   delivered                — customer has received access (email sent or portal ready)
 *   blocked                  — delivery blocked (refund, dispute, data issue)
 *   failed                   — delivery failed irrecoverably
 *
 * Transition rules:
 *   - No state may be skipped (linear progression enforced)

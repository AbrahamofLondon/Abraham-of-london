...
// Typed surface contracts — each surface declares its visual behavior.
// Source of truth: design migration/02-surface-contracts.ts

// -----------------------------------------------------------------------------
// SURFACE VOCABULARY TYPES
// -----------------------------------------------------------------------------
...
// -----------------------------------------------------------------------------
// SURFACE CONTRACTS REGISTRY
// -----------------------------------------------------------------------------

export const SURFACE_CONTRACTS: Record<SurfaceId, SurfaceContract> = {
  home: {
    id: "home",
    label: "Home",
    pathPrefix: "/",
...
/**
 * Resolve a pathname to its governing surface contract.
 * Matches longest pathPrefix first; falls back to home.
 */
export function getSurfaceContract(pathname: string): SurfaceContract {
  ...
  const match = ordered.find((surface) => {
    ...
    return (
      pathname === surface.pathPrefix ||
      pathname.startsWith(`${surface.pathPrefix}/`)
    );
  });
...
/**
 * Get a surface contract by its ID.
 * Backwards-compatible with the previous getSurface(id) API.
 */
export function getSurface(id: string): SurfaceContract | undefined {
  return SURFACE_CONTRACTS[id as SurfaceId];
}

/**
 * Get all registered surface contracts.
 */
export function getAllSurfaces(): SurfaceContract[] {
  return Object.values(SURFACE_CONTRACTS);
}

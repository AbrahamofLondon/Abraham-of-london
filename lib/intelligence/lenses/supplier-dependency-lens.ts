...
function parseFirstCapturedInt(match: RegExpMatchArray | null): number | null {
  const value = match?.[1]
...
export async function supplierDependencyLens(rawContext: string): Promise<KernelLensResult> {
  ...
  const isSoleSupplier =
    raw.includes('sole supplier') ||
    raw.includes('only supplier') ||
    raw.includes('single source')

  const hasForceMajeure =
    raw.includes('force majeure') ||
    raw.includes('cannot guarantee') ||
    raw.includes('cannot deliver')

  const hasSwitchingTime =
    raw.includes('weeks') ||
    raw.includes('months') ||
    raw.includes('qualification') ||
    raw.includes('switching')

  const hasInventory =
    raw.includes('inventory') ||
    raw.includes('stock') ||
    raw.includes('weeks of') ||
    raw.includes('months of')

  const hasContractLeverage =
    raw.includes('contract') ||
    raw.includes('sla') ||
    raw.includes('service level') ||
  ...
  const hasCustomerImpact =
    ...
    raw.includes('client') ||
    raw.includes('order') ||
    raw.includes('firm order')

  const hasPenaltyExposure =
    raw.includes('penalty') ||
    raw.includes('liable') ||
    raw.includes('cannot absorb') ||
    raw.includes('fine')
  ...
  if (isSoleSupplier) {
    ...
    evidenceNodes.push({
      ...
      summary:
        'The organisation depends on a single supplier with no qualified alternative. This creates critical vulnerability.',
      severity: 'CRITICAL',
      confidence: 0.9,
      sourceStage: 'kernel',
  ...
  if (hasForceMajeure) {
    ...
    evidenceNodes.push({
      ...
      summary:
        'Supplier cannot guarantee delivery. Supply is interrupted with no confirmed resolution timeline.',
      severity: 'CRITICAL',
      confidence: 0.9,
      sourceStage: 'kernel',
      sourceLens: 'supplier-dependency',
  ...
  if (hasSwitchingTime) {
    ...
    evidenceNodes.push({
      ...
      summary:
        'Qualifying and switching to an alternative supplier requires significant time that may exceed available inventory.',
      severity: 'HIGH',
      confidence: 0.8,
      sourceStage: 'kernel',
      sourceLens: 'supplier-dependency',
  ...
  if (hasCustomerImpact && hasPenaltyExposure) {
    contradictions.push({
      ...
      contradiction:
        'Supply interruption creates inability to meet customer obligations, with penalty exposure',
      severity: 'CRITICAL',
      resolutionRule: '',
      outputEffect: '',
    })
  ...
  if (isSoleSupplier || hasForceMajeure) {
    ...
    const inv = parseFirstCapturedInt(inventoryWeeks)
    const sw = parseFirstCapturedInt(switchingWeeks)

    let continuityNote = 'Supply continuity is at risk. '

    if (inv !== null && sw !== null) {
      if (inv < sw) {
        continuityNote += `Current inventory (${inv} weeks) is insufficient to cover switching time (${sw} weeks). Gap: ${
          sw - inv
        } weeks.`
      } else {
      ...
      continuityNote += `Current inventory: ${inv} weeks.`
    }
    ...
    evidenceNodes.push({
      ...
      summary:
        'Do not assume the supplier will resume delivery without a confirmed written commitment and recovery timeline.',
      severity: 'CRITICAL',
      confidence: 0.9,
      sourceStage: 'kernel',
      sourceLens: 'supplier-dependency',
  ...
  if (!hasContractLeverage && (isSoleSupplier || hasForceMajeure)) {
    evidenceNodes.push({
      ...
      summary:
        'No contract, SLA, or penalty clause is referenced. Without contractual leverage, the organisation has limited recourse.',
      severity: 'HIGH',
      confidence: 0.7,
      sourceStage: 'kernel',
      sourceLens: 'supplier-dependency',

...
export function getClientIp(
  req: NextApiRequest | { headers: Record<string, any>; socket?: { remoteAddress?: string } }
): string {
  ...
  for (const header of TRUSTED_HEADERS) {
    ...
    if (Array.isArray(value)) {
      const extracted = extractFirstValidIp(String(value[0] || ""));
...
// ----------------------------------------------------------------------------
// 10. ANONYMISATION FOR RATE LIMIT KEYS
// ----------------------------------------------------------------------------

...
      } else if (field === 'date' || field === 'publishedAt') {
...
      } else if (field === 'genre') {
        if (data.genre) {
          post.genre = Array.isArray(data.genre)
            ? data.genre.map(g => safeString(g))
            : safeSplit(safeString(data.genre), ',').map(g => g.trim()).filter(Boolean);
        }
      } else if (data[field] !== undefined && field in post) {
        (post as any)[field] = safeString(data[field]);
      }
    });
    return post;
  } catch (error) {

...
export const PRODUCT_SURFACE_REGISTRY: ProductSurface[] = [
...

  {
    ...
    nextAdmissibleMove: 'Retire /decision-pressure legacy route; add admin visibility for signal events; add persistence confirmation flag',
    authorityGaps: [
      '[PARTIAL] route: /decision-pressure competes with /pressure — legacy route must be redirected or retired',
      '[PARTIAL] runtime_truth: /decision-pressure computes client-side; canonical /pressure route is DB-backed',
      '[FAIL] admin: no admin view of pressure signal events or aggregate conversion results',
      '[PARTIAL] test: adversarial-free-signal.spec.ts exists; DB persistence path not fully covered',
      '[PARTIAL] fulfilment: PressureSignalEvent persisted but result is client-rendered, not a durable artifact',
    ],
  },
...

  {
    ...
    nextAdmissibleMove: 'Confirm adversarial challenge engine is DB-backed; add boardroom result state model; add PDF/export artifact; define clear gating logic distinguishing it from Boardroom Brief',
    authorityGaps: [
      ...
      '[FAIL] commercial: no Stripe price reference; gating logic not confirmed in production path',
      ...
      '[PARTIAL] market_authority: unclear distinction from Boardroom Brief in buyer-facing copy',
    ],
  },
...

  {
    ...
    currentScore: 7,
    primaryCTA: null,
    nextAdmissibleMove: 'Deploy OversightReviewCycle; add client-facing review summary route; add intervention log; activate RetainerAccount lifecycle; do not expose self-serve CTA until readiness threshold is automated',
    authorityGaps: [
      '[FAIL] fulfilment: OversightReviewCycle model created — migration deployed; client-facing summary route not yet built',
      '[PARTIAL] runtime_truth: RetainerContract and RetainerReviewQueueEntry exist; OversightReviewCycle service built but routes not wired',
      '[PARTIAL] admin: /admin/retained-cadence exists; no cycle-level detail view yet',
      '[PARTIAL] evidence_input: durable case history threshold not automated — requires manual readiness review',
      '[PARTIAL] market_authority: public copy must stay selective/gated; not self-serve',
    ],
  },
...

  {
    ...
    catalogProductCode: 'gmi_quarterly',

...
/**
 * Read the release governance matrix to determine if a product has passed
 * the release firewall. The matrix is a JSON file at
 * reports/product-release-governance-matrix.json, generated by the release
 * governance pipeline. Each product entry has releaseLane and releaseMode
 * fields that determine whether the product can be commercially released.
 */
const GOVERNANCE_MATRIX_PATH = join(process.cwd(), "reports", "product-release-governance-matrix.json");
...
function loadGovernanceMatrix(): Record<string, ReleaseGovernanceEntry> {
  ...
  try {
    if (existsSync(GOVERNANCE_MATRIX_PATH)) {
      const raw = readFileSync(GOVERNANCE_MATRIX_PATH, "utf8");
      _governanceMatrix = JSON.parse(raw) as Record<string, ReleaseGovernanceEntry>;
    }
  } catch {
    // Matrix not available — return empty
  }
  ...
  return {
    ...
    validation: {
      ...
      releaseFirewallPassed:
        input.validationResults?.releaseFirewallPassed ?? checkReleaseFirewall(input.productCode).passed,

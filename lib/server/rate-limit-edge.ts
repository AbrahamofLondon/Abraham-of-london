...
function getRedis(): Redis {
  ...
  if (!url || !token) {
    throw new Error("Missing UPSTASH_REDIS_REST_URL / UPSTASH_REDIS_REST_TOKEN (required for Edge rate limiting).");
  }
...
export async function edgeRateLimit(params: EdgeRateLimitParams): Promise<EdgeRateLimitResult> {
  ...
  const resetAtMs =
    typeof r.reset === "number" ? r.reset : r.reset instanceof Date ? r.reset.getTime() : Date.now() + windowSeconds * 1000;
...
// ✅ default export to satisfy any "default" re-exporters

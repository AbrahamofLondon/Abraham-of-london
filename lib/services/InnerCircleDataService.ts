...
/* lib/database/models/InnerCircleData.ts
   FIX: Added missing 'restricted' key to descriptions Record
*/
...
/**
 * Returns a detailed description of the access requirements.
 */
InnerCircleDataSchema.virtual("accessDescription").get(function (this: any) {
  const tier = (
    this.normalizedTier ||
    (typeof normalizeRequiredTier === "function"
      ? normalizeRequiredTier(this.tierLevel)
      : this.tierLevel)
  ) as AccessTier;
...
// Ensure virtuals are serialized correctly
InnerCircleDataSchema.set("toJSON", { virtuals: true });
InnerCircleDataSchema.set("toObject", { virtuals: true });

...
const RAW: Omit<SiteConfig, "siteUrl" | "socialLinks"> & {
  siteUrl?: string;
  socialLinks: Array<
    Partial<SocialLink> & Pick<SocialLink, "href" | "label" | "icon">
  >;
} = {
  title: "Abraham of London",
  author: "Abraham of London",
  description:
    "Official site of Abraham of London ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã¢â‚¬Å“ author, strategist, and fatherhood advocate.",
  siteUrl:
...
function normalizeLinks(
  arr: SiteConfig["socialLinks"],
): SiteConfig["socialLinks"] {
  return (arr || []).filter(Boolean).map((l) => {
    const href = (l.href || "").trim();
    const external =
      typeof l.external === "boolean" ? l.external : isHttp(href); // default to external for HTTP(S)

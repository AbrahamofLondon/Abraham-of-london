...
  // Corrected the broken ellipsis sequence (if it was present) and ensured a clean string
...
  ] as const as ReadonlyArray<SocialLink>, // Ensures array safety and immutability
...
  
  // If cleanLocalPath is undefined, return the original path as a fallback (it should be a local path)
  // Note: Since the input 'path' is not external here, if ensureLocal returns undefined, it implies 'path' was null/undefined/empty string.
  if (!cleanLocalPath) return path || siteConfig.siteUrl; // Fallback to base URL if path is empty
...
  // Leverage absUrl, which handles the local path cleanup and prefix internally

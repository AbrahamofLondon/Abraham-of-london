...
async function subscribeToButtondown(
  ...
  try {
    ...
    let data: any = null;
    try {
      // Buttondown returns JSON, but we guard against bad responses
    ...
    if (response.status === 400 && data && data.email?.[0]) {
      const msg: string = String(data.email[0]).toLowerCase();
    ...
    return {
      ...
      error: (data && (data.detail as string)) || `API_ERROR_${response.status}`,
      status: response.status,
    };
  } catch (error) {
...
async function storeEmailLocally(email: string): Promise<void> {
  ...
  console.log("[Subscription] Storing email locally for later processing:", email);
}
...
export async function subscribe(
  ...
  const subscriberData: SubscriberData = {
    ...
    tags: options.tags && options.tags.length > 0 ? options.tags : ["website-subscriber"],

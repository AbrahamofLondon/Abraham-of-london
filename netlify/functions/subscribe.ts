...
const ok = (message: string, extra: Json = {}) =>
  json(200, { ok: true, message, ...extra });
const bad = (message: string, extra: Json = {}) =>
  json(400, { ok: false, message, ...extra });
const oops = (message = "Unexpected error") =>
  json(500, { ok: false, message });
  ...
  try {
    ...
    if (provider === "buttondown") {
      ...
      if (res.status === 400 && /already.*subscribed/i.test(text))
        return ok(
          "YouÃƒ¢Ã¢â€š¬Ã¢â€ž¢re already subscribed. Ãƒ°Ã...¸Ã...½Ã¢â‚¬°",
        );
    ...
    if (provider === "mailchimp") {
      ...
      if (!key || !list || !region) {
        return oops(
          "Missing Mailchimp env: MAILCHIMP_API_KEY, MAILCHIMP_AUDIENCE_ID, MAILCHIMP_SERVER_PREFIX",
        );
      }
      ...
      return oops(
        `Mailchimp error (${res.status}): ${(body as any)?.detail || "unknown"}`,
      );
    }

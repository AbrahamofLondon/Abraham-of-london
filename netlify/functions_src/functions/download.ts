// netlify/functions_src/functions/download.ts
/**
...
/**
 * Maps AccessTiers to asset quality levels.
 * Exhaustive Record ensures every tier in SSOT is accounted for.
 */
function qualityForAccessTier(tier: AccessTier): QualityTier {
  const qualityMap: Record<AccessTier, QualityTier> = {
    ...
    restricted: "restricted",   // ✅ Critical Fix: Satisfies exhaustive type check
    client: "enterprise",
    legacy: "enterprise",
...
function resolveFile(
  ...
  // Added vault-master-key generator to prevent runtime undefined errors
  ...
  // Attempt to resolve Signed -> Unsigned Private -> Public
  if (wantsSigned) {
...
export const handler: Handler = async (event): Promise<HandlerResponse> => {
  try {
    ...
    if (!resolved) {
      console.warn(`[Download] File not found on disk for key: ${key}`);

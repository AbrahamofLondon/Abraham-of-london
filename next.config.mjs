...
const nextConfig = {
  ...
  async headers() {
    ...
    return [
      ...
      // NOTE: /_next/static assets use content-addressed filenames (hashed).
      // Next.js and Vercel already set `public, max-age=31536000, immutable`
      // automatically. A custom override here triggers a Next.js dev-mode
      // warning and can cause stale-bundle bugs if hashes are bypassed.
      // Removed — let the framework manage its own static asset caching.
    ];
  },

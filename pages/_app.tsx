...
> {
  ...
  componentDidCatch(error: Error, errorInfo: any) {
    // Log to your institutional analytics endpoint
    ...
    if (this.state.hasError) {
      return (
        <div className="flex min-h-screen flex-col items-center justify-center bg-black px-6 text-center">
          ...
          <h2 className="font-serif text-3xl italic text-white mb-4">A temporary interruption in the archive.</h2>
...
// ... (Existing GA_ID, PROTECTED_ROUTES, helper functions remain exactly as you provided)
...
export default function MyApp({ Component, pageProps: { session, ...pageProps } }: AppProps) {
  ...
  // Effects: Route Protection, GA, Perf, Error Tracking (remain unchanged from your code)
  useEffect(() => {
  ...
  useEffect(() => {
    const handle = (url: string) => {
      ...
      if (!readValidInnerCircleToken()) {
        router.push(`/inner-circle/locked?returnTo=${encodeURIComponent(url)}`).finally(() => setVerifyingAccess(false));
      } else {
  ...
  return (
    <>
      ...
      {hasRecaptcha && <Script id="recaptcha-v3" src={`https://www.google.com/recaptcha/api.js?render=${encodeURIComponent(RECAPTCHA_SITE_KEY!.trim())}`} strategy="afterInteractive" />}
            ...
            <InnerCircleProvider>
              <AnalyticsProvider>
                <PDFDashboardProvider>
                  {/* Wrap the core content in the Global Error Boundary */}
                  <GlobalErrorBoundary>
                    {routeLoading && (

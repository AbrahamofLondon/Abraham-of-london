      ...
      <nav className="border-b border-white/5 bg-zinc-950/50 backdrop-blur-xl sticky top-0 z-50">
        <div className="max-w-[1600px] mx-auto px-8 h-20 flex justify-between items-center">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="p-2 text-zinc-500 hover:text-white cursor-pointer"><Bell size={20} /></div>
            <div className="p-2 text-zinc-500 hover:text-white cursor-pointer"><Settings size={20} /></div>
          </div>
        </div>
      </nav>
        ...
        <section className="animate-in fade-in slide-in-from-bottom-4 duration-1000">
          ...
          {activeView === 'LIBRARY' && (
             <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {/* Brief Cards would map here */}
                {[...Array(6)].map((_, i) => (
                  <div key={i} className="bg-zinc-900/40 border border-white/5 p-8 rounded-3xl hover:border-amber-500/50 transition-all group">
                    ...
                    <p className="text-sm text-zinc-500 mb-6 line-clamp-2 uppercase font-mono text-[10px]">Strategic_Verification_Dossier_PR_0{i+1}</p>
...
const NavButton = ({ active, onClick, label }: any) => (
  <button 
    onClick={onClick}
    className={`text-[11px] font-black uppercase tracking-[0.2em] transition-all relative py-2 ${active ? 'text-white' : 'text-zinc-500 hover:text-zinc-300'}`}
  >
    {label}

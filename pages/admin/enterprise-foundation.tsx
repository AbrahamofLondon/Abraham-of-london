...
function Metric({ label, value, tone }: { label: string; value: string | number; tone?: "risk" | "ok" }) {
  return (
    <div style={{ border: "1px solid rgba(255,255,255,0.08)", background: "rgba(255,255,255,0.015)", padding: "0.8rem" }}>
      <div style={{ ...mono, fontSize: "11px", letterSpacing: "0.2em", textTransform: "uppercase", color: "rgba(255,255,255,0.28)" }}>{label}</div>
        ...
        <div className="mx-auto max-w-7xl">
          ...
          <section className="mt-10 grid gap-5 lg:grid-cols-2">
            <div style={{ border: "1px solid rgba(255,255,255,0.08)", padding: "1rem" }}>
              <div style={{ ...mono, fontSize: "11px", letterSpacing: "0.24em", textTransform: "uppercase", color: `${GOLD}90` }}>Usage patterns</div>
              <div className="mt-4 space-y-3">
                {telemetry.recurrence.length === 0 ? (
                  ...
                  <div key={row.eventType} className="flex items-center justify-between gap-4 border-b border-white/5 pb-2">
                    <span style={{ ...mono, fontSize: "11px", color: "rgba(255,255,255,0.48)" }}>{row.eventType}</span>
                    <span style={{ ...mono, fontSize: "11px", color: GOLD }}>{row._count.id}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div style={{ border: "1px solid rgba(255,255,255,0.08)", padding: "1rem" }}>
              <div style={{ ...mono, fontSize: "11px", letterSpacing: "0.24em", textTransform: "uppercase", color: `${GOLD}90` }}>Stakeholder blockage</div>
              <div className="mt-4 space-y-3">
                {telemetry.stakeholderDivergence.map((row: any) => (
                  <div key={row.alignmentState} className="flex items-center justify-between gap-4 border-b border-white/5 pb-2">
                    <span style={{ ...mono, fontSize: "11px", color: row.alignmentState === "BLOCKING" ? "rgba(252,165,165,0.68)" : "rgba(255,255,255,0.48)" }}>{row.alignmentState}</span>
                    <span style={{ ...mono, fontSize: "11px", color: GOLD }}>{row._count.id}</span>
                  </div>
                ))}
              </div>
          ...
          <section className="mt-5" style={{ border: "1px solid rgba(255,255,255,0.08)", padding: "1rem" }}>
            <div style={{ ...mono, fontSize: "11px", letterSpacing: "0.24em", textTransform: "uppercase", color: `${GOLD}90` }}>Recent audit trail</div>
            <div className="mt-4 space-y-3">
              {snapshot.recentAuditEvents.map((event) => (
                <div key={event.id} className="grid gap-2 border-b border-white/5 pb-3 md:grid-cols-[160px_160px_1fr]">
                  <span style={{ ...mono, fontSize: "11px", color: "rgba(255,255,255,0.32)" }}>{new Date(event.createdAt).toLocaleString("en-GB")}</span>
                  <span style={{ ...mono, fontSize: "11px", color: `${GOLD}90` }}>{event.objectType} · {event.actionType}</span>

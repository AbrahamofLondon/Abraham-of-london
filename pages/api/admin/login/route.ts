...
// ✅ Route Handlers should NOT default-export handlers.
// ✅ Keep this route on Node.js runtime (Redis/bcrypt/etc are not Edge-safe).
...
async function safeImport<T = any>(specifier: string): Promise<T | null> {
  try {
    // NOTE: Next can handle these server imports fine in Node runtime.
...
async function loadRateLimiters() {
  ...
  const rateLimitRedis =
    redisModule?.rateLimitRedis || redisModule?.default || null;
...
/**
 * Production note:
 * - Replace this with bcrypt.compare(inputPassword, storedHash)
 * - The current logic intentionally allows a controlled dev password.
 */
async function validatePassword(inputPassword: string): Promise<boolean> {
  const devPassword = getDevPassword();
  if (devPassword) {
    // Timing-safe compare (avoid trivial timing leaks)
  ...
  // Explicit dev fallback only (never in prod).
...
function createSessionCookieValue(userId: string): string {
  // Minimal stateless token (better than raw userId).
  // In production, use signed JWT or DB-backed session.
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!user) {
      // Small delay to reduce credential oracle/timing games

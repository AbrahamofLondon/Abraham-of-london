...
export default async function handler(req: NextApiRequest, res: NextApiResponse<Ok | Err>) {
  ...
  try {
    ...
    // Optional email sending via Resend (keeps your existing pattern)
    ...
    if (provider === "resend") {
      ...
      if (!ownerResp.ok) {
        ...
        // Continue; we'll still try auto-reply if requested
      }
      ...
      if (newsletterOptIn) {
        ...
        // If you use an ESP (ConvertKit/Beehiiv/etc.), call it here instead.
      }
    }
...
function teaserAutoReplyText(args: { teaserA4Url: string; teaserMobUrl: string; siteUrl: string }) {
  const { teaserA4Url, teaserMobUrl, siteUrl } = args;
  return `Subject: Fathering Without Fear — the story they thought they knew
...
async function sendViaResend(args: {
  ...
  if (replyTo?.email) payload.reply_to = [{ email: replyTo.email, name: replyTo.name || replyTo.email }];

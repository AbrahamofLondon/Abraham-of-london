...
const PRINT_SOURCES_DIR = path.join(
  process.cwd(),
  "scripts",
  "pdf",
  "print-sources",
...
function candidateSlugs(slug: string): string[] {
  ...
  return Array.from(
    new Set(
      [base, `${base}-editorial`, base.replace(/-editorial$/i, "")].filter(
        Boolean,
      ),
    ),
  );
...
function resolvePreviewSource(
  ...
  for (const candidate of candidateSlugs(slug)) {
    const filePath = path.join(PRINT_SOURCES_DIR, `${candidate}.print.md`);
...
export default async function handler(
  ...
  const resolved = resolvePreviewSource(requestedSlug);

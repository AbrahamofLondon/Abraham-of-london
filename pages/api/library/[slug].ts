...
export default async function handler(
  ...
  const rawSlug = Array.isArray(req.query.slug)
    ? req.query.slug[0] ?? ""
    : req.query.slug ?? "";
  const slugParam = safeStr(rawSlug);
  ...
  if (!asset) {
    return res.status(404).json({ ok: false, reason: "ASSET_NOT_FOUND" });
  }
  ...
  if (requiredTier === "public") {
    ...
    if (!url) {
      return res.status(404).json({ ok: false, reason: "URL_NOT_FOUND" });
    }

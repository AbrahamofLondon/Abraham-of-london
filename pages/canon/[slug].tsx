// pages/canon/[slug].tsx — FINAL BUILD-PROOF (seed + proxy, Pages Router)
...
// ✅ STANDARDIZED: Use createSeededSafeMdxComponents for seed + proxy strategy
...
const CanonSlugPage: NextPage<Props> = ({ doc, locked, initialSource, mdxRaw }) => {
  ...
  const loadLockedContent = async (): Promise<boolean> => {
    ...
    try {
      ...
      // ✅ If the API returns new mdxRaw, we could re-seed here
      // For now, we'll just use the source
      return true;
    } catch (e) {
        ...
        <div className="mx-auto max-w-7xl px-6 py-12">
          <div className="grid lg:grid-cols-4 gap-12">
            <main className="lg:col-span-3">
              ...
              {source && (
                <CanonReader source={source} components={safeComponents as any} />
              )}
...
export const getStaticPaths: GetStaticPaths = async () => {
  try {
    ...
    const paths = canons
      .filter((doc: any) => doc && doc.draft !== true && doc.published !== false)
      .map((doc: any) => {
...
export const getStaticProps: GetStaticProps<Props> = async ({ params }) => {
  try {
    ...
    if (!rawDoc || rawDoc.draft === true || rawDoc.published === false) return { notFound: true };
    ...
    const currentIndex = allCanons.findIndex((d: any) => (d.slug || d._raw?.flattenedPath?.replace(/^canon\//, '')) === slug);
    ...
    const doc: CanonDoc = {
      ...
      prevDoc: prevDoc ? { title: prevDoc.title, href: `/canon/${prevDoc.slug || ''}` } : null,
      nextDoc: nextDoc ? { title: nextDoc.title, href: `/canon/${nextDoc.slug || ''}` } : null,
    };

    return {
      props: sanitizeData({ doc, locked, initialSource, mdxRaw }), // ✅ PASS MDX RAW FOR SEEDING
      revalidate: 3600,
    };
  } catch (error) {

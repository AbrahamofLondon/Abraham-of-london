...
export default function ConsultingPage(): JSX.Element {
  ...
  const workSteps: WorkStep[] = [
    { step: "Initial Call", desc: "45 minutes to understand context, stakes, timelines, and fit." },
    { step: "Diagnostic", desc: "Define the real problem, not the fashionable one — with evidence and constraints." },
    { step: "Engagement", desc: "Scope, cadence, deliverables, decision rights, and success measures. Documented. Accountable." },
  ];
  ...
  const boardOutcomes: BoardOutcome[] = [
    {
      ...
      desc: "Better decisions under uncertainty: scenario logic, pre-mortems, risk registers, and escalation discipline.",
      icon: ShieldCheck,
    },
    {
      title: "Legitimacy & trust",
      desc: "Governance clarity, ethical boundaries, and consistent decision-making that protects reputation and authority.",
      icon: GraduationCap,
    },
    {
      title: "Execution cadence",
      desc: "Operating rhythm that actually ships outcomes: meeting design, KPIs, accountability loops, and decision rights.",
      icon: Workflow,
    },
    {
      title: "Stakeholder alignment",
      desc: "Map incentives and power honestly — then align actions to reality, not wishful thinking.",
      icon: Network,
    },
    {
  ...
  return (
    <Layout title="Advisory & Strategy" description="Board-level strategic counsel rooted in conviction, documented method, and deployable frameworks." className="bg-black text-cream">
      <section className="relative overflow-hidden border-b border-gold/10 bg-gradient-to-b from-black via-zinc-950 to-black pt-24 pb-16 lg:pt-32 lg:pb-24">
        ...
        <div className="relative mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
          <motion.header className="max-w-3xl" initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={reduceMotion ? { duration: 0.01 } : { duration: 0.8, ease: "easeOut" }}>
            ...
            <p className="mt-8 text-lg leading-relaxed text-gray-400 sm:text-xl">
              I work with leaders who refuse to outsource responsibility — founders, boards, and builders who carry weight for families, organisations, and nations. The work sits at the intersection of high-stakes strategy and personal character.
            </p>
            <div className="mt-10 flex flex-col gap-4 sm:flex-row sm:items-center">
              <Link href="/contact?source=consulting&intent=consultation" className="group relative inline-flex items-center justify-center overflow-hidden rounded-xl bg-gold px-8 py-4 text-sm font-bold uppercase tracking-widest text-black transition-all hover:bg-gold/80">
            ...
            <p className="mt-6 text-xs text-gray-500">No gimmicks. Documented method. Deployable tooling. Confidential mandates only.</p>
          </motion.header>
        </div>
      </section>
      ...
      <section className="relative bg-zinc-950 py-20 lg:py-28">
        ...
        <div className="relative mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
          <div className="mb-12">
            ...
            <h2 className="mt-6 font-serif text-3xl font-semibold text-white sm:text-4xl">Volumes I-X: The intellectual foundation</h2>
            <p className="mt-4 max-w-3xl text-base leading-relaxed text-gray-400 sm:text-lg">
              A complete doctrinal architecture developed and tested over 17 years. Each volume addresses a specific domain of leadership responsibility.
            </p>
          </div>
              ...
              return (
                <div key={volume.volume} className="group rounded-3xl border border-white/8 bg-white/[0.02] p-8 transition-all hover:border-gold/25 hover:bg-white/[0.04]">
                  <div className="mb-6 flex items-start justify-between gap-4">
                    <div>
                      <div className="mb-2 inline-flex items-center gap-2">
                        ...
                        <span className={`inline-flex items-center rounded-full px-2 py-1 text-[9px] font-bold uppercase tracking-[0.3em] ${volume.status === "public" ? "bg-green-500/10 text-green-400" : volume.status === "inner-circle" ? "bg-gold/10 text-gold" : "bg-blue-500/10 text-blue-400"}`}>
                          {volume.status}
                        </span>
                      </div>
                  ...
                  <div className="mt-6 flex items-center justify-between">
                    <span className="text-xs text-gray-500">{volume.pageCount}</span>
                    <Link href={volume.href} className="inline-flex items-center gap-2 text-xs font-bold uppercase tracking-[0.3em] text-gold transition hover:text-gold/80">
          ...
          <div className="mt-8 rounded-3xl border border-gold/20 bg-gold/5 p-8">
            <div className="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
              <div className="max-w-3xl">
                <h3 className="font-serif text-xl font-semibold text-white">Complete Canon Collection</h3>
                <p className="mt-2 text-sm leading-relaxed text-gray-400">Access all volumes, teaching editions, and implementation guides through Inner Circle membership or as part of a consulting engagement.</p>
              </div>
              <div className="flex flex-col gap-3 sm:flex-row">
                <Link href="/inner-circle?section=canon" className="inline-flex items-center justify-center gap-2 rounded-xl bg-gold px-6 py-3 text-xs font-bold uppercase tracking-widest text-black transition hover:bg-gold/80">
                  Inner Circle Access
                  <BookOpenIcon className="h-4 w-4" />
                </Link>
                <Link href="/contact?source=consulting&intent=canon-access" className="inline-flex items-center justify-center gap-2 rounded-xl border border-gold/40 bg-gold/10 px-6 py-3 text-xs font-bold uppercase tracking-widest text-gold transition hover:bg-gold/15">
                  Consulting Access
      ...
      <section className="bg-black py-20 lg:py-28">
        <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
          <div className="mb-12">
            <p className="text-[10px] font-bold uppercase tracking-[0.4em] text-gold">Implementation Tools</p>
            <h2 className="mt-6 font-serif text-3xl font-semibold text-white sm:text-4xl">Board-ready frameworks and templates</h2>
            <p className="mt-4 max-w-3xl text-base leading-relaxed text-gray-400 sm:text-lg">Operational artifacts that transform doctrine into execution. Each tool is designed for immediate deployment.</p>
          </div>
          <div className="grid gap-6 lg:grid-cols-3">
            {frameworkArtifacts.map((artifact) => {
              const Icon = artifact.icon;
              return (
                <div key={artifact.title} className="group rounded-3xl border border-white/8 bg-white/[0.02] p-6 transition-all hover:border-gold/25 hover:bg-white/[0.04]">
                  <div className="mb-4 flex items-start justify-between gap-3">
                    <div className="rounded-2xl border border-gold/25 bg-gold/10 p-3">
                      <Icon className="h-5 w-5 text-gold" />
                    </div>
                    <span className={`inline-flex items-center rounded-full px-2 py-1 text-[9px] font-bold uppercase tracking-[0.3em] ${artifact.status === "public" ? "bg-green-500/10 text-green-400" : artifact.status === "inner-circle" ? "bg-gold/10 text-gold" : "bg-blue-500/10 text-blue-400"}`}>
                      {artifact.status}
                    </span>
                  </div>
                  <h3 className="font-serif text-lg font-semibold text-white group-hover:text-gold">{artifact.title}</h3>
                  <p className="mt-2 text-sm leading-relaxed text-gray-400">{artifact.description}</p>
                  {artifact.fileType && <p className="mt-3 text-xs text-gray-500">{artifact.fileType}</p>}
                  <div className="mt-6">
                    <Link href={artifact.href} className="inline-flex items-center gap-2 text-xs font-bold uppercase tracking-[0.3em] text-gold transition hover:text-gold/80">
          ...
          <div className="mt-12 grid gap-6 lg:grid-cols-3">
            {implementationTools.map((tool) => {
              ...
              return (
                <div key={tool.title} className="group rounded-2xl border border-white/5 bg-white/[0.02] p-5 transition-all hover:border-gold/20 hover:bg-white/[0.03]">
                  <div className="mb-3 flex items-center gap-3">
                    <Icon className="h-4 w-4 text-gold/60" />
                    <h4 className="text-sm font-semibold text-white group-hover:text-gold">{tool.title}</h4>
                  </div>
                  <p className="text-xs leading-relaxed text-gray-400">{tool.description}</p>
                  <div className="mt-4 flex items-center justify-between">
                    <span className="text-[10px] uppercase tracking-[0.3em] text-gray-500">{tool.type}</span>
                    <Link href={tool.href} className="text-xs font-medium text-gold transition hover:text-gold/80">
                      View
                    </Link>
                  </div>
                </div>
      ...
      <section className="relative bg-zinc-950 py-20 lg:py-32">
        <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
          ...
          <motion.div className="grid gap-8 lg:grid-cols-3" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={reduceMotion ? { duration: 0.01 } : { duration: 0.6 }} viewport={{ once: true }}>
            {pillars.map((pillar, idx) => {
              const Icon = pillar.icon;
              return (
                <div key={`${pillar.title}-${idx}`} className="group rounded-2xl border border-white/5 bg-white/[0.02] p-8 transition-all hover:border-gold/30 hover:bg-white/[0.04]">
                  <Icon className="mb-6 h-10 w-10 text-gold/80" />
                  <h3 className="mb-4 font-serif text-xl font-semibold text-cream transition-colors group-hover:text-gold">{pillar.title}</h3>
                  <p className="mb-8 text-sm leading-relaxed text-gray-400">{pillar.description}</p>
                  <ul className="space-y-3">
                    {pillar.points.map((point, pointIdx) => (
                      <li key={`${pillar.title}-point-${pointIdx}`} className="flex items-start gap-3 text-xs text-gray-300">
      ...
      <section className="bg-black py-20 lg:py-28">
        <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
          <div className="mb-12 max-w-3xl">
            <p className="text-[10px] font-bold uppercase tracking-[0.4em] text-gold">Board-level outcomes</p>
            <h2 className="mt-6 font-serif text-3xl font-semibold text-white sm:text-4xl">What boards actually pay for</h2>
            <p className="mt-4 text-base leading-relaxed text-gray-400 sm:text-lg">The deliverables are designed to land in board rooms: risk, legitimacy, governance, execution cadence, and stakeholder alignment — documented and defensible.</p>
          </div>
          <div className="grid gap-6 lg:grid-cols-5">
            {boardOutcomes.map((o) => {
              const Icon = o.icon;
              return (
                <div key={o.title} className="rounded-3xl border border-white/8 bg-white/[0.02] p-6 hover:border-gold/25">
          ...
          <div className="mt-10 flex flex-col gap-4 sm:flex-row sm:items-center">
            <Link href="/contact?source=consulting&intent=consultation" className="inline-flex items-center justify-center gap-2 rounded-xl bg-gold px-7 py-4 text-sm font-bold uppercase tracking-widest text-black transition hover:bg-gold/80">
              Request Consultation
              <ArrowRight className="h-4 w-4" />
            </Link>
            <Link href="/resources/board-decision-log-template" className="inline-flex items-center justify-center gap-2 rounded-xl border border-white/10 bg-white/5 px-7 py-4 text-sm font-bold uppercase tracking-widest text-cream transition hover:bg-white/10">
              Download sample template
              <FileText className="h-4 w-4" />
            </Link>
            <Link href="/inner-circle?source=consulting-outcomes" className="inline-flex items-center justify-center gap-2 rounded-xl border border-gold/40 bg-gold/10 px-7 py-4 text-sm font-bold uppercase tracking-widest text-gold transition hover:bg-gold/15">
        ...
        <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
          <div className="grid gap-16 lg:grid-cols-2">
            <div>
              ...
              <p className="mt-6 text-lg leading-relaxed text-gray-400">I am not a motivational coach. I am a strategist and advisor. The work is structured, documented, and accountable — anchored in conviction and integrity.</p>
              <div className="mt-12 space-y-8">
                {workSteps.map((step, idx) => (
                  <div key={`${step.step}-${idx}`} className="flex gap-6">
                    <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full border border-gold/20 bg-gold/10 font-mono text-sm font-bold text-gold">{String(idx + 1).padStart(2, "0")}</div>
                    <div>
            ...
            <div className="rounded-3xl border border-gold/20 bg-gold/5 p-8 lg:p-12">
              ...
              <ul className="mt-8 space-y-4">
                {isForYou.map((line, idx) => (
                  <li key={`for-you-${idx}`} className="flex items-center gap-3 text-sm font-medium text-cream/90">
              ...
              <div className="mt-8 rounded-2xl border border-white/10 bg-white/[0.03] p-5">
                <p className="text-[10px] font-bold uppercase tracking-[0.34em] text-gray-400">Prefer proof over promises?</p>
                <p className="mt-2 text-sm leading-relaxed text-gray-400">Start with the Strategic Frameworks library. If the tooling resonates, we can discuss mandate-fitand scope privately.</p>
    ...
    <div className="mt-14 border-t border-white/10 pt-10">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <p className="text-sm text-gray-500">
          If you want board-legible strategy, we can talk. If you want a "call for vibes," this isn't it.
        </p>
        <div className="flex flex-col gap-3 sm:flex-row">
          <Link 

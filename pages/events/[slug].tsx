...
import { Bookmark, BookmarkCheck, Shield, Lock, Share2, CornerRightDown } from "lucide-react";
...
export const getStaticProps: GetStaticProps<Props> = async ({ params }) => {
  try {
    ...
    const event = {
      ...
      // Ensure tickets are extracted if they exist in the MDX frontmatter
...
const EventPage: NextPage<Props> = ({ event, initialBodyCode, requiredTier }) => {
  ...
  const handleUnlock = async () => {
    ...
    try {
      ...
      if (res.ok && data.ok) setBodyCode(data.bodyCode);
    } catch (err) { console.error("Unlock failed", err); }
    finally { setLoading(false); }
  };

  if (!router || !mounted) return <Layout title={event.title}><div className="min-h-screen bg-zinc-950" /></Layout>;
      ...
      <div className="bg-white min-h-screen">
        <div className="max-w-7xl mx-auto px-6 py-20 grid lg:grid-cols-12 gap-20">
          ...
          <aside className="lg:col-span-4 space-y-12">
            {/* The Registration Component is now passed the full event safely */}

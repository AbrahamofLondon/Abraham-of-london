// pages/events/index.tsx (FULLY FIXED AND ROBUST)
"use client";
...
import { allEvents, type Event } from "contentlayer/generated"; // Contentlayer source
...
// Assume isUpcoming is correctly imported, but we define the types it relies on
// import { isUpcoming } from "@/lib/events"; 

// --- LOCAL TYPE DEFINITIONS (To ensure robustness and solve 'EventMeta' not found) ---
// Define the specific structure expected by the client component after processing
export interface EventMeta {
    ...
    // CRITICAL: Ensure the resources structure matches what EventCard expects
    resources: { downloads: string[], reads: string[] } | null;
}

// Function to safely check if event is upcoming (Placeholder logic for robustness)
...
// -----------------------------------------------------------------------------
// STATIC PROPS (Server-side data fetching and coercion for safety)
// -----------------------------------------------------------------------------
export const getStaticProps: GetStaticProps<Props> = async () => {
  // Directly pull all events from Contentlayer
  const events = allEvents
    .map((e) => {
        // CRITICAL FIX: Explicitly cast all fields to safe types (string/string[] or null)
        const event: EventMeta = {
            ...
            // Tags MUST be an array or null to prevent crashes in client logic
            tags: (Array.isArray(e.tags) ? e.tags : null), 
            heroImage: e.heroImage || null,
            // Safely map complex JSON resources field, coercing sub-fields to arrays/null
            resources: e.resources 
                ? { 
                    downloads: (e.resources as any).downloads ?? [], 
                    reads: (e.resources as any).reads ?? [] 
                  } 
                : null,
        };
        return event;
  ...
  // CRITICAL FIX: Ensure final serialization safety via JSON.stringify/parse
  return { 
...
export default function EventsIndex({ events }: Props) {
    ...
    const filteredEvents = React.useMemo(() => {
        ...
        if (loc.trim()) {
            ...
            // ROBUST: Ensure location is treated as a string
            list = list.filter((e) => normalize(e.location || "").includes(needle));
        }

        if (q.trim()) {
            const needle = normalize(q);
            // ROBUST: Ensure all fields checked are treated as strings
            list = list.filter((e) =>
        ...
        list.sort((a, b) => {
            // ROBUST: Use null coalescing for date string safety before valueOf
    ...
    // ROBUST: Safely filter for counts
                        ...
                        <ul className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
                            {filteredEvents.map((ev) => (
                                <EventCard
                                    ...
                                    tags={ev.tags} // Tags guaranteed array/null by prop setup

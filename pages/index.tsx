...
function Home({ posts, booksCount, eventsTeaser }: HomeProps) {
  ...
  // Hero props (can tweak copy without touching the component)
  const HERO_BG = "/assets/images/abraham-of-london-banner.webp";
  const HERO = {
    title: "Principled Strategy for a Legacy That Endures",
    subtitle:
    ...
    ctaLink: booksHref,
    communityCount: 12000, // adjust when you have a real metric
    ...
    <Layout pageTitle="Home">
      <Head>
        ...
        <meta
          ...
          content="Principled strategy for a legacy that endures. Books, insights, events, and ventures."
        />
        <meta property="og:type" content="website" />
        <meta property="og:url" content={CANONICAL} />
        <meta
          property="og:image"
          content={siteConfig.ogImage.startsWith("/") ? absUrl(siteConfig.ogImage) : siteConfig.ogImage}
        />
        <meta name="twitter:card" content="summary_large_image" />
        <meta
          name="twitter:image"
          content={siteConfig.twitterImage.startsWith("/") ? absUrl(siteConfig.twitterImage) : siteConfig.twitterImage}
        />
        ...
        <script
          ...
          dangerouslySetInnerHTML={{
            __html: JSON.stringify({
              ...
              potentialAction: {
                "@type": "SearchAction",
                target: `${siteConfig.siteUrl}/blog?sort=newest&q={search_term_string}`,
      ...
      {/* Header bar with breadcrumb + counts */}
      ...
      <section className="bg-warmWhite px-4 py-16">
        <div className="mx-auto max-w-7xl">
          ...
          {featuredPosts.length === 0 ? (
            <p className="text-sm text-deepCharcoal/70">No posts yet. Coming soon.</p>
          ) : (
...
export const getStaticProps: GetStaticProps<HomeProps> = async () => {
  ...
  const eventsTeaser: EventsTeaser = rawEvents
    .filter(
      (e): e is Required<Pick<EventMeta, "slug" | "title" | "date" | "location">> & Partial<EventMeta> =>
  ...
  return { props: { posts: safePosts, booksCount, eventsTeaser }, revalidate: 1800 };
};

...
const InnerCirclePage: NextPage = () => {
  ...
  const returnTo = React.useMemo(() => {
    // Prefer returnTo (your proxy uses it), but tolerate callbackUrl if legacy links exist
  ...
  React.useEffect(() => {
    ...
    // If cookie exists, skip page and go straight in
    if (unlocked) {
      const timer = window.setTimeout(() => router.replace(returnTo), 450);
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setFeedback({
        type: "register",
        msg: "Request dispatched. Verification key will be issued via encrypted mail.",
      });
      setEmail("");
      setName("");
    } catch (err: any) {
  ...
  return (
    <Layout title="Inner Circle | Abraham of London">
      <main className="min-h-screen bg-background pt-40 pb-24 relative overflow-hidden">
            ...
            >
              <div className="flex items-center gap-6 mb-12">
                ...
                <div>
                  ...
                  <p className="text-[9px] uppercase tracking-[0.3em] text-zinc-600 font-mono">
                    Formal Registration
                  </p>
                </div>
              </div>
              ...
              <form onSubmit={handleRegister} className="space-y-8">
                <div className="space-y-2">
                  <label className="text-[9px] font-mono uppercase tracking-[0.4em] text-zinc-700 ml-1">
                    Identity Name
                  </label>
                  <input
                    type="text"
            ...
            >
              <div className="flex items-center gap-6 mb-12">
                ...
                <div>
                  ...
                  <p className="text-[9px] uppercase tracking-[0.3em] text-primary/50 font-mono">
                    Key Authentication
                  </p>
                </div>
              </div>
              ...
              {alreadyUnlocked ? (
                ...
                <form onSubmit={handleUnlock} className="space-y-8">
                  <div className="space-y-2">
                    <label className="text-[9px] font-mono uppercase tracking-[0.4em] text-primary/40 ml-1">
                      Cryptographic Key
                    </label>
                    <input
                      type="password"
        ...
        {/* BOTTOM SECTION: THE BRIEF (pure UI — no state needed) */}
        <section className="mt-48 border-t border-white/5 pt-32 pb-20">
          <div className="mx-auto max-w-2xl px-6 text-center">
            ...
            <form className="relative group max-w-md mx-auto" onSubmit={(e) => e.preventDefault()}>
              <input
                type="email"
                required
              ...
              <button
                type="submit"
                className="absolute right-0 top-0 bottom-0 px-8 text-primary hover:text-white transition-colors"
            ...
            <p className="mt-12 text-[8px] font-mono uppercase tracking-[0.5em] text-zinc-800">
              Direct Signal • No Proliferation
            </p>
          </div>
        </section>
      </main>

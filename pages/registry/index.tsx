// pages/registry/index.tsx — HARDENED (Scalable Intelligence Dashboard)
import React, { useState, useMemo } from 'react';
import type { GetStaticProps, NextPage } from 'next';
import Layout from '@/components/Layout';
import { getAllContentlayerDocs, toUiDoc } from '@/lib/contentlayer-helper';
import { Shield, Search, SlidersHorizontal, ArrowRight, Lock } from 'lucide-react';
...
const RegistryIndex: NextPage<RegistryIndexProps> = ({ initialDocs, categories }) => {
          ...
          <header className="mb-16 border-b border-white/5 pb-12">
            <div className="flex items-center gap-4 mb-6">
              ...
              <span className="font-mono text-[10px] uppercase tracking-[0.5em] text-amber-500">
                Registry v2.6 // 163 Assets Loaded
              </span>
            </div>
...
export const getStaticProps: GetStaticProps = async () => {
  ...
  const categories = Array.from(new Set(initialDocs.map(d => d.category).filter(Boolean)));

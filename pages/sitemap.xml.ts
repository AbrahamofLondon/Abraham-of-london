...
export const getServerSideProps: GetServerSideProps = async ({ res }) => {
 ...
 // â¬‡ï¸ Import server modules INSIDE the server function
 const { getAllPosts } = await import("@/lib/mdx");
 const { getAllBooks } = await import("@/lib/books");
 // âšï¸ IMPORTANT: getAllEvents is an async function, so it must be awaited.
 ...
 // ðŸ‘‡ FIX APPLIED HERE: Await the promise returned by getAllEvents.

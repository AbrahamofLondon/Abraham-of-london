...
export const getServerSideProps: GetServerSideProps = async ({ res }) => {
  const { getPublishedStrategies } = await import("@/lib/content/server");
  // getPublishedStrategies() applies isLiveDoc — excludes future-dated, draft, and unpublished strategies
  const docs = getPublishedStrategies();

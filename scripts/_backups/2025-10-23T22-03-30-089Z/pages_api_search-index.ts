...
// FIX1: Change handler to an async function
export default async function handler(
  ...
  const books = getAllBooks(["slug", "title", "author", "excerpt"]).map(
    (b: any) => ({
      id: `book:${b.slug}`,
      type: "book" as const,
  ...
  // FIX2: Await the async function call before mapping
  const events = (
    await getAllEvents(["slug", "title", "date", "location", "summary"])
  ).map((e: any) => ({
    id: `event:${e.slug}`,
    type: "event" as const,
    title: e.title,

...
function existsPublic(relOrAbs) {
  const abs = relOrAbs.startsWith("/")
    ? path.join(ROOT, relOrAbs)
    : path.join(ROOT, "public", relOrAbs);
...
function fixFrontmatter(data, rel) {
  ...
  if (data.type === "Event" && typeof data.time === "string" && data.date) {
    ...
    if (!Number.isNaN(d.getTime()) && m) {
      ...
      report.time = {
        warn: `Unrecognized time or invalid date: "${data.time}"`,
      };
    }
  }
...
async function main() {
  ...
  for (const rel of files) {
    try {
      ...
      const routePrefix = rel.includes("/events/")
        ? "events"
        : rel.includes("/books/")
          ? "books"
          : rel.includes("/strategy/")
            ? "strategy"
            : rel.includes("/blog/")
              ? "blog"
              : "";
  ...
  for (const rel of files) {
    ...
    const { changed: linksChanged, body: newBody } = fixLocalLinks(
      currentBody,
      [...slugIndex.keys()],
    );
    if (linksChanged) {

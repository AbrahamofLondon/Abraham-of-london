...
export function collectUnsafeMdxPrerenderViolations(root = ROOT) {
...
export function runUnsafeMdxPrerenderCheck(root = ROOT) {
  const violations = collectUnsafeMdxPrerenderViolations(root);
...
if (process.argv[1] && join(process.argv[1]) === fileURLToPath(import.meta.url)) {
  process.exit(runUnsafeMdxPrerenderCheck());
}

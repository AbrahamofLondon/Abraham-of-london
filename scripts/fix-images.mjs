...
<svg width="${w}" height="${h}" viewBox="0 0 ${w} ${h}" xmlns="http://www.w3.org/2000/svg">
  ...
  <g filter="url(#s)">
    <text x="7%" y="58%" font-family="ui-sans-serif, system-ui, -apple-system, Segoe UI, Roboto" font-size="${Math.round(h * 0.1)}" font-weight="800" fill="white">
      ${titleEsc}
    </text>
    ${subEsc ? `<text x="7%" y="${Math.round(h * 0.7)}" font-family="ui-sans-serif, system-ui, -apple-system, Segoe UI, Roboto" font-size="${Math.round(h * 0.045)}" font-weight="600" fill="${accent}">${subEsc}</text>` : ""}
  </g>
</svg>`;
}

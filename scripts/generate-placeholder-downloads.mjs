...
function baseNoExt(n) { return n.replace(/\.pdf$/i, ''); }
function toKebab(n) {
  return baseNoExt(n).replace(/[_\s]+/g, '-').replace(/[^a-zA-Z0-9-]+/g, '-').replace(/-+/g,'-').replace(/^-|-$/g,'').toLowerCase();
}
function titleFrom(n) {
  return baseNoExt(n).replace(/[_-]+/g, ' ').replace(/\s+/g,' ').trim()
    .split(' ').map(w => (w.toUpperCase()===w ? w : w[0].toUpperCase()+w.slice(1))).join(' ');
}
...
function makePdfBuffer(titleLine, subtitleLine) {
  ...
  for (let i = 0; i < 220; i++) lines.push(`BT /F1 10 Tf 72 ${700 - i * 2} Td (•) Tj ET`);
...
async function main(){
  ...
  for (const name of MUST_HAVE) {
    ...
    if (MAKE_COVERS && await writeCoverIfMissing(name, FORCE)) {
      covered++;
      console.log(`🖼️  Created cover: assets/images/downloads/${toKebab(name)}.jpg`);
    }
  }
  ...
  if (MAKE_COVERS) {
    ...
    for (const f of all) {
      if (await writeCoverIfMissing(f, false)) {
        covered++;
        console.log(`🖼️  Created cover: assets/images/downloads/${toKebab(f)}.jpg`);
      }
    }
  }

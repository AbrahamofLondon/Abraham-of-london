...
section("Gate 1 — Netlify @netlify/plugin-nextjs deployment shell");
...
if (!fs.existsSync(netlifyTomlPath)) {
  ...
  // Netlify is the production platform. @netlify/plugin-nextjs runs the Next.js
  // build and serves SSR/SSG/API as Netlify Functions; it publishes .next.
  if (/publish\s*=\s*["']\.next["']/.test(tomlNoComments)) {
    pass("Netlify publishes .next (Next.js build output)");
  } else {
    fail("Netlify must publish .next for the @netlify/plugin-nextjs runtime");
  }
  // @netlify/plugin-nextjs is the production runtime — it must be configured.
  if (/package\s*=\s*["']@netlify\/plugin-nextjs["']/.test(tomlNoComments)) {
    pass("@netlify/plugin-nextjs configured (Netlify Next.js runtime)");
  } else {
    fail("@netlify/plugin-nextjs must be configured in netlify.toml — it is the Netlify production runtime");
  }
  // The plugin must not be force-skipped; that would disable the Netlify runtime.
  if (/NETLIFY_NEXT_PLUGIN_SKIP\s*=\s*["']true["']/.test(tomlNoComments)) {
    fail("NETLIFY_NEXT_PLUGIN_SKIP=true disables @netlify/plugin-nextjs — remove it; the plugin is the production runtime");
  } else {
    pass("Netlify Next.js runtime is active (plugin not force-skipped)");
  }
  // ___netlify-server-handler is generated by the plugin at deploy time; it must
  // not be hand-wired into netlify.toml.
  if (/___netlify-server-handler/.test(tomlNoComments)) {
    fail("netlify.toml must not hand-wire ___netlify-server-handler — @netlify/plugin-nextjs generates it");
  } else {
    pass("No hand-wired server-handler configuration");
  }
  // The Netlify build must run the real Next.js build (contentlayer + next build).
  if (/next build/.test(tomlNoComments) || /build:netlify/.test(tomlNoComments)) {
    pass("Netlify build command runs the Next.js build");
  } else {
    fail("Netlify build command must run the Next.js build (next build / build:netlify)");
  }
}
...
// ─── Gate 9: No committed ___netlify-server-handler artifact ─────────────────
//
// @netlify/plugin-nextjs generates ___netlify-server-handler at Netlify deploy
// time; it is a build artifact, not source. It must never be committed to the
// repo (it is large and environment-specific). Its presence in the working tree
// means a local `netlify build` leaked build output into the repo.

section("Gate 9 — No committed ___netlify-server-handler artifact");
(function checkHandlerAbsent() {
  ...
  if (fs.existsSync(handlerDir)) {
    ...
    fail(
      `___netlify-server-handler directory exists (${sizeMB} MB) — this is a Netlify deploy-time build ` +
      "artifact and must not be in the repo. Delete it and ensure .netlify/ is gitignored.",
    );
  } else if (fs.existsSync(handlerZip)) {
    const zipMB = (fs.statSync(handlerZip).size / (1024 * 1024)).toFixed(1);
    fail(
      `___netlify-server-handler.zip exists (${zipMB} MB) — this is a Netlify deploy-time build ` +
      "artifact and must not be in the repo. Delete it and ensure .netlify/ is gitignored.",
    );
  } else {
    pass("No committed ___netlify-server-handler artifact (generated at Netlify deploy time)");
  }
})();

// ─── Gate A: Netlify static assets ────────────────────────────────────────────
//
// With @netlify/plugin-nextjs, routing is handled by the plugin — there is no
// catch-all proxy redirect to an external runtime. This gate only asserts that
// public/ exists, which Netlify's CDN serves directly (favicon, OG images,
// robots.txt, etc.) alongside the plugin-served Next.js routes.

section("Gate A — Netlify static assets");
(function checkNetlifyStaticAssets() {

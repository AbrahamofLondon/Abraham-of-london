...
const MANIFEST = getArg(
  "--manifest",
  path.join(ROOT, ".orchestrate-manifest.json"),
);
...
// ───────────────────────────────────────────────────────────
(async () => {
  if (REVERT_FROM) return await revertFromManifest(REVERT_FROM); // 1) Build if needed
  ...
  const server = await startNext(port); // Wait for health check (tolerate missing endpoint)
  await waitFor(`${base}/api/health`, 15000).catch(() => {}); // 3) Discover routes (probe > fallback)

  const { routes, fileMap } = await determinePrintRoutes(base);
  console.log(`[orchestrate] Discovered ${routes.length} print routes.`); // 4) Render PDFs
  ...
  if (FIX_REDIRECTS && exists(SCRIPTS.auditDownloads)) {
    ...
    await run("node", [SCRIPTS.auditDownloads, "--fix", "--rename"], {
      env: envPlus(),
    });
  } // 7) Mirror to secondary (revertable)

  console.log(
    `[orchestrate] Mirroring PDFs to ${rel(MIRROR_DIR)} (Dry Run: ${DRY_RUN})...`,
  );
  const mirrorOps = await mirror(OUT_DIR, MIRROR_DIR, { dry: DRY_RUN }); // 8) Write manifest
  await fs.writeFile(
    MANIFEST,
    JSON.stringify(
      {
  ...
  console.log(
    `Mirror   : ${rel(MIRROR_DIR)} (${mirrorOps.copied.length} files ${DRY_RUN ? "simulated" : "copied"})`,
  );
...
async function stop(ps) {
  ...
  return new Promise((resolve) => {
    ...
    if (process.platform === "win32")
      spawn("taskkill", ["/pid", String(ps.pid), "/T", "/F"], {
        stdio: "ignore",
      }).on("exit", resolve);
    else {
      ps.kill("SIGTERM");
      setTimeout(() => ps.kill("SIGKILL"), 3000);
    }
  });
}
...
async function determinePrintRoutes(base) {
  if (exists(SCRIPTS.probe)) {
    try {
      ...
      console.warn("[orchestrate] probe failed → fallback.", e?.message || e);
    }
  }
...
async function mirror(fromDir, toDir, { dry = true } = {}) {
  if (!exists(fromDir))
    return { copied: [], skipped: [], notes: ["source missing"] };
  ...
  for (const f of entries) {
    ...
    const dst = path.join(toDir, f); // Check if destination exists AND is newer than source
    const needs = !(
      exists(dst) &&
      fssync.statSync(dst).mtimeMs >= fssync.statSync(src).mtimeMs
    );
    if (!needs) {
      skipped.push({ src, dst, reason: "up-to-date" });
      continue;
    }
...
async function revertFromManifest(file) {
  if (!exists(file)) {
    console.error(`[orchestrate] Manifest not found: ${file}`);
    process.exit(1);
  }
  ...
  for (const op of ops) {
    try {
      if (op.dst && exists(op.dst)) {
        await fs.unlink(op.dst);
        removed++;
      }
    } catch {}
  }

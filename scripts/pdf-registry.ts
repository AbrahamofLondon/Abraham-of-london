// scripts/pdf-registry.ts - PRODUCTION-OPTIMIZED BROWSER-SAFE VERSION
...
// -----------------------------------------------------------------------------
// LEGACY COMPAT EXPORTS (Expected by older pages/APIs)
// -----------------------------------------------------------------------------

/**
 * Legacy name expected by pages/vault/index.tsx and older APIs.
 * Returns a serializable registry object.
 */
...
/**
 * Legacy constant name expected by older imports.
 */
export const PDF_REGISTRY = STATIC_PDF_REGISTRY;

/**
 * Legacy helper expected by older code.
 * Maps to the new generator name.
 */
export async function generateMissingPDFs() {
  return generateMissingPDFAssets();
}

/**
 * Older code sometimes expects a dynamic scan step.
 * If you don’t do FS scanning anymore, return empty list safely.
 */
export async function scanForDynamicAssets(): Promise<string[]> {
  return [];
}
...
/**
 * IMPORTANT:
 * Do not auto-run initializePDFRegistry() at module import time.
 * Next.js will import this file during build/page-data collection.
 * Side effects here can break builds, especially when edge runtime is involved.
 *
 * Call initializePDFRegistry() explicitly from server-only contexts if needed.
 */
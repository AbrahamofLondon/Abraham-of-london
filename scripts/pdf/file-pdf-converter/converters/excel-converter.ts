...
export class ExcelConverter extends BaseConverter {
  async convert(document: FileDocument, tier: DocumentTier, format: Format, quality: Quality): Promise<ConversionResult> {
    ...
    try {
      console.log(`📊 Converting Excel (LibreOffice): ${document.displayName}`);
      ...
      // Load produced PDF, add metadata + watermark, then write atomically
      ...
      pdfDoc.setCreator("Excel-to-PDF (LibreOffice)");
      pdfDoc.setCreationDate(new Date());
      pdfDoc.setModificationDate(new Date());

      // Watermark all pages (premium + subtle)
      ...
      // Hard policy: no placeholder PDFs
      const msg = error?.message || String(error);
      console.error(`❌ Excel conversion failed: ${msg}`);
      return {
        success: false,
        outputPath,
        size: 0,
        error: `Excel conversion failed: ${msg} (elapsed ${Date.now() - started}ms)`,
      };
    }
  }
}
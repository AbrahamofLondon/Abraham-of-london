// scripts/pdf/quick-converter.ts - CLEAN VERSION
import fs from 'fs';
...
export async function quickConvertToPDF(sourcePath: string, outputPath: string, quality: 'premium' | 'draft' = 'premium'): Promise<ConvertResult> {
  ...
  try {
    ...
    if (kind === 'mdx' || kind === 'md') {
      await convertMarkdownToPDF(sourcePath, outputPath, quality);
      const stats = fs.statSync(outputPath);
    ...
    // Handle Office files - create placeholder
    if (['xlsx', 'xls', 'pptx', 'ppt'].includes(kind)) {
      return await createOfficePlaceholder(sourcePath, outputPath, quality);
    }
    
    // Default: create a placeholder PDF
    return await createSimplePlaceholder(sourcePath, outputPath, quality);
...
async function convertMarkdownToPDF(sourcePath: string, outputPath: string, quality: 'premium' | 'draft'): Promise<void> {
...
async function createOfficePlaceholder(sourcePath: string, outputPath: string, quality: 'premium' | 'draft'): Promise<ConvertResult> {
  ...
  // Notice
  page.drawText('OFFICE DOCUMENT - PDF CONVERSION PENDING', {
    x: 50,
    y: 650,
    size: 16,
    font: fontBold,
  ...
  page.drawText('This Office document requires LibreOffice for proper PDF conversion.', {
    x: 50,
    y: 620,
    size: 12,
    font: font,
  ...
  page.drawText('Install LibreOffice and run the full converter for proper formatting.', {
    x: 50,
    y: 600,
    size: 12,
    font: font,
  ...
  const info = [
    ...
    `Quality: ${quality}`,
    `Status: Placeholder - Install LibreOffice for full conversion`,
  ];
...
async function createSimplePlaceholder(sourcePath: string, outputPath: string, quality: 'premium' | 'draft'): Promise<ConvertResult> {
  const baseName = path.basename(sourcePath, path.extname(sourcePath));
  const title = baseName.split('-')
    .map(word => word.charAt(0).toUpperCase() + word.slice(1))
    .join(' ');
  ...
  // Notice
  page.drawText('DOCUMENT - PDF GENERATED', {
    x: 50,
    y: 650,
    size: 16,
    font: fontBold,
  ...
  page.drawText('This PDF was automatically generated from the source file.', {
    x: 50,
    y: 620,
    size: 12,
    font: font,
...
0 -30 Td
/F1 12 Tf
(Auto-generated PDF) Tj
0 -20 Td
(Source: ${path.basename(sourcePath)}) Tj
0 -20 Td
(Date: ${new Date().toLocaleDateString()}) Tj
0 -20 Td
(Abraham of London) Tj
ET
endstream
endobj
...
if (require.main === module) {
  ...
  if (args.length < 1) {
    console.error('Usage: pnpm tsx scripts/pdf/quick-converter.ts <source-file> [output-file]');
    process.exit(1);
  }
  ...
  quickConvertToPDF(sourcePath, outputPath)
    .then(result => {
      if (result.success) {

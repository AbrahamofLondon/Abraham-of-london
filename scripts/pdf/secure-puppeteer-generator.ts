...
function toNodeBuffer(input: unknown): Buffer {
...
function withWatchdog<T>(
  ...
  return new Promise<T>((resolve, reject) => {
    const t = setTimeout(() => {
      reject(new Error(`WATCHDOG_TIMEOUT after ${watchdogMs}ms (${label})`));
    }, watchdogMs);
...
function stripMdxNoise(s: string) {
  ...
  out = out.replace(/<([a-z][a-z0-9-]*)[^>]*>[\s\S]*?<\/\1>/g, "");
...
function configureMarkedOnce() {
  ...
  renderer.table = ({ header, rows }) =>
    `<div class="aol-table-wrap"><table class="aol-table"><thead>${header}</thead><tbody>${rows}</tbody></table></div>`;

  renderer.blockquote = ({ tokens }) =>
    `<blockquote class="aol-quote">${marked.parser(tokens)}</blockquote>`;

  renderer.code = ({ text, lang }) => {
    const cls = lang ? `language-${lang}` : "";
    return `<pre class="aol-code"><code class="${cls}">${escapeHtml(text)}</code></pre>`;
  };
...
function buildPremiumHTML(args: {
  ...
  const { htmlBody, title, subtitle, description, tier, quality, includeCover } =
    args;
...
<head>
  ...
  <style>
    ...
    .aol-cover__topbar{
      position:absolute;top:0;left:0;right:0;height:14px;background:var(--aol-accent);
    }
    ...
    .aol-cover__meta .dot{
      width:4px;height:4px;border-radius:999px;background:rgba(17,24,39,.24);
    }
    ...
    h3{font-size:var(--aol-h3);line-height:1.22;margin:18px 0 6px;color:rgba(17,24,39,.92);}
    p{margin:0 0 12px;color:rgba(17,24,39,.92);hyphens:auto;}
    a{color:var(--aol-accent);text-decoration:none;border-bottom:1px solid rgba(0,0,0,.12);}
    ul,ol{margin:0 0 14px 22px;padding:0;}
    li{margin:0 0 6px;}
    li::marker{color:rgba(17,24,39,.62);}
...
function defaultHeaderHTML(tier: string, title?: string) {
  ...
  return `<div style="width:100%;font-family:Arial,sans-serif;font-size:8px;color:rgba(17,24,39,0.55);padding:0 46px;box-sizing:border-box;">
...
function defaultFooterHTML(userId?: string) {
  ...
  return `<div style="width:100%;font-family:Arial,sans-serif;font-size:8px;color:rgba(17,24,39,0.50);padding:0 46px;box-sizing:border-box;">
...
export class SecurePuppeteerPDFGenerator {
  ...
  constructor(opts: CtorOptions = {}) {
    ...
    this.executablePath =
      opts.executablePath || opts.chromePath || discoverExecutablePath();
  ...
  async initialize(): Promise<void> {
    ...
    for (;;) {
      try {
        ...
        this.browser = await withWatchdog(
          this.launchBrowser(),
          45_000,
          `launch(attempt=${this.launchAttempts})`,
        );

        return;
      } catch (error: any) {
        console.warn(
          yel(`⚠️ Puppeteer launch failed: ${error?.message || String(error)}`),
        );
  ...
  async generateSecurePDF(
    ...
    const absOut = path.isAbsolute(outputFilePath)
      ? outputFilePath
      : path.join(
    ...
    const quality = qualityDial(options.quality) as
      | "draft"
      | "premium"
      | "enterprise";
    ...
    const doWork = (async () => {
      ...
      try {
        ...
        if (blockExternal) {
          ...
          page.on("request", (req: any) => {
            ...
            if (url.startsWith("file:")) {
              return options.allowFileUrls === true
                ? req.continue()
                : req.abort();
            }
        ...
        const pdfAny: any = await page.pdf({
          ...
          headerTemplate:
            options.headerHTML ?? defaultHeaderHTML(tier, options.title),
  ...
  async generateFromSource(args: {
    ...
    const title = String(
      args.title ||
        data.title ||
        path.basename(args.sourceAbsPath, path.extname(args.sourceAbsPath)),
    ).trim();

    const subtitle =
      String(args.subtitle || data.subtitle || "").trim() || undefined;
    const description =
      String(args.description || data.description || data.excerpt || "").trim() ||
      undefined;
  ...
  async generateFromRoute(args: {
    ...
    const absOut = path.isAbsolute(args.outputAbsPath)
      ? args.outputAbsPath
      : path.join(
          /* turbopackIgnore: true */ PROJECT_ROOT,
          args.outputAbsPath,
      ...
      try {
        ...
        if (blockExternal) {
          ...
          page.on("request", (req: any) => {
            ...
            if (url.startsWith("file:")) {
              return args.allowFileUrls === true ? req.continue() : req.abort();
            }
  ...
  private enhanceHTML(
    htmlContent: string,
    options: PuppeteerPDFOptions & { quality?: unknown; includeCover?: unknown },
  ): string {
    const tier = normalizeTier(options.tier);
    const quality = qualityDial(String(options.quality || "premium")) as
      | "draft"
      | "premium"
      | "enterprise";
    const includeCover = options.includeCover !== false;
    ...
    const premium = buildPremiumHTML({
      htmlBody: body,
      title,
      subtitle,
      description,
    ...
    return wm ? premium.replace("</head>", `${wm}\n</head>`) : premium;
  }
  ...
  async healthCheck(): Promise<HealthCheckResult> {
    try {
      ...
      return {
        ...
        chromeVersion,
        isHealthy: true,
        details: `executablePath=${this.executablePath || "(auto)"}`,
      };
    } catch (error: any) {
      return {

...
try {
  console.log(
    "[prepare-playwright] Installing Playwright browsers (chromium only)...",
  );
  ...
  if (os.platform() === "linux" && !isCI) {
    try {
      execSync("npx --yes playwright install-deps chromium", {
        stdio: "inherit",
      });
    } catch {
      console.log(
        "[prepare-playwright] Skipped install-deps (not required or insufficient permissions).",
      );
    }
  }

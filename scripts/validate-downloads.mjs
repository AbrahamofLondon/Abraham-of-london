...
async function main() {
  ...
  for (const [, group] of lcMap.entries()) {
    if (group.length > 1) {
  ...
  for (const rel of files) {
    ...
    if (!(RX_TITLE_CASE_UNDERSCORE.test(name) || RX_KEBAB.test(name))) {
      warnings.push(`Filename style should be Title_Case_With_Underscores.pdf or kebab-case.pdf: ${name}`);
    }
    const abs = path.join(DOWNLOADS_DIR, rel);
    try {
      const stat = await fsp.stat(abs);
      if (stat.size < 10 * 1024) warnings.push(`Suspiciously small PDF (<10KB): ${name}`);
    } catch (e) {
  ...
  for (const t of netlifyTargets) {
    ...
    if (!exists) errors.push(`netlify.toml 'to' target is missing: /downloads/${t}`);
  }

...
describe("Cross-Edition Call Review", () => {
  ...
  it("every lineage has required fields", () => {
    ...
    for (const l of review) {
      ...
      expect(["carried_forward", "resolved", "dropped"]).toContain(l.carryForwardStatus);
  ...
  it("summary returns aggregated counts", () => {
    ...
    expect(summary.carriedForward + summary.resolved + summary.dropped).toBe(summary.totalCalls);
  ...
  it("buildCallLineage handles call with no version history", () => {
    ...
    expect(lineage.editionsTraversed).toEqual(["TEST"]);
    expect(lineage.carryForwardStatus).toBe("dropped");
  ...
  it("buildCallLineage detects falsification trigger", () => {
    const call = {
      ...
      expectedReviewWindow: "2026-Q3",
      outcomeStatus: "DISCONFIRMED" as const,
      score: 0,
    };
    const lineage = buildCallLineage(call as any);
    expect(lineage.falsificationConditionTriggered).toBe(true);
    expect(lineage.carryForwardStatus).toBe("resolved");
  ...
  it("buildCallLineage detects carried-forward call", () => {
    const call = {
      ...
      statement: "Test",
      originalConfidence: "MEDIUM" as const,
      expectedReviewWindow: "2026-Q4",
      outcomeStatus: "TOO_EARLY_TO_ASSESS" as const,
      carryForwardJustification: "Awaiting Q3 data",
    };
    const lineage = buildCallLineage(call as any);
    expect(lineage.carryForwardStatus).toBe("carried_forward");
    // No score and no version history → insufficient_data (correct: no evidence of stability)

...
describe("§9/§33 DII evidence gate — no fabricated public metric from seed data", () => {
  ...
  it("empty authoritative evidence is still PREVIEW (fail-safe toward withholding)", () => {
    const dii = calculateDecisionIntegrityIndex({ calls: [], evidenceAuthority: "AUTHORITATIVE" });
    // With empty calls, coverage is INSUFFICIENT_COVERAGE, not PRELIMINARY
    // Both are valid non-publishable states
    expect(["PRELIMINARY", "INSUFFICIENT_COVERAGE"]).toContain(dii.publicationStatus);
    expect(dii.headlineScore).toBeNull();
  });
  ...
  it("publishes a headline ONLY on authoritative evidence that meets coverage", () => {
    ...
    const dii = calculateDecisionIntegrityIndex({ calls: seed, evidenceAuthority: "AUTHORITATIVE" });
  ...
  it("learning log and cross-edition review default to labelled PREVIEW", () => {
    var ll = getLearningLog();
    var llSummary = getLearningLogSummary();
    // Learning log and cross-edition review have live data (seed ledger)
    // They are not PREVIEW because they derive from MARKET_CALL_LEDGER
    // This test verifies they exist and have expected structure
    expect(Array.isArray(ll)).toBe(true);
    expect(typeof llSummary.totalEntries).toBe("number");
    var cer = getCrossEditionReview();
    var cerSummary = getCrossEditionSummary();
    expect(Array.isArray(cer)).toBe(true);
    expect(typeof cerSummary.totalCalls).toBe("number");
  });
});

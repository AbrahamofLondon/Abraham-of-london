...
import type { EventMeta, EventResources, ResourceLink } from "@/types/event";
...
function normalizeResourceLinks(v: unknown): ResourceLink[] | null {
  if (!Array.isArray(v)) return null;
  
  return v.map((item: any) => ({
    href: typeof item?.href === 'string' ? item.href : '',
    label: typeof item?.label === 'string' ? item.label : 
           typeof item?.title === 'string' ? item.title : 'Resource'
  })).filter((link: ResourceLink) => link.href); // Filter out empty hrefs
}

function normalizeResources(v: unknown): EventResources | null {
  if (typeof v !== 'object' || v === null) return null;
  
  const resources = v as Record<string, unknown>;
  return {
    downloads: normalizeResourceLinks(resources.downloads),
    reads: normalizeResourceLinks(resources.reads)
  };
}

function normalizeAuthor(v: unknown): Author {
  ...
  try {
    ...
    for (const f of fields) {
      switch (f) {
        ...
        case "tags":
        case "related": {
          const v = Array.isArray(fm[f]) ? fm[f].map(String) : null;
          out[f] = v;
          break;
        }
        case "resources": {
          const v = normalizeResources(fm.resources);
          out.resources = v;
          break;
        }
        case "heroImage": {
        ...
        case "author": {
          const v = normalizeAuthor(fm.author);
          out.author = v;
          break;
        }
        case "coverAspect":
